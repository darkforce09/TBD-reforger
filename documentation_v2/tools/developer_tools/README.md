...
- `enf`: Enfusion `.pak` unpacker, script extractor, and symbol citation verifier.
- `mcpd`: Workbench NetAPI broker daemon operating over an `AF_UNIX` domain socket.
- `world`: World object processor and 512m chunk spatial partitioner.
- `map`: 2D satellite orthophoto optimizer and tile pyramid compiler.
