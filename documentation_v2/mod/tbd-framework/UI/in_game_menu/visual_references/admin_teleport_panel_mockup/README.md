...
# Admin teleport panel mockup

Design-phase reference for the admin menu's Teleport module: moving players and vehicles. It gives layout and colour context and is not an implementation source; the built UI is the
[EnfScript](/documentation_v2/glossary/a_to_f.md#enfscript) code the Code section links.

## Contents

```text
documentation_v2/mod/tbd-framework/UI/in_game_menu/visual_references/admin_teleport_panel_mockup/
├── admin_teleport_panel_mockup.html  the Stitch export
└── admin_teleport_panel_mockup.png   its screenshot
```

## How it works

The set shows "Source (Who to Move)" with a filtered list of players, squads and vehicles, "Destination (Where to Send)" with players, objectives ("Objective: North Outpost"), squads and bases, and "Transfer Directives" with the calculated separation between them.
