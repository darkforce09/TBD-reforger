...
/// T-935.8 — reading a blueprint back out of `prefabs/building_blueprints.rkyv`.
///
/// The archive's blueprint (spec §4) is the **tactical** subset: the vertical profile and, per
/// level, the walls, doors, windows, stairs and furniture the LOS and viewshed passes attribute
/// hits to. It deliberately drops the presentation and provenance halves — `overall_footprint`,
/// the `roof` and `plate` grids, `floor_polygons`, every `prefab_resource`, the door hinge/swing
/// fields, `glass_pane_count`, furniture `size2_d`, level `name` / `slice_height_m`,
/// `chimney_height_m` — so a value from [`BuildingBlueprint::from_archived`] carries those at
/// their empty defaults. `evaluate_los` reads none of them for attribution; the `/debug/building-
/// viewer` bench and the roof view do, and they keep loading the JSON.
///
/// Numbers are `f32` on the wire, so a round-tripped coordinate equals the JSON one only after
/// `f64 → f32 → f64`. The parity test over the committed goldens does exactly that rather than
/// carry a tolerance.
#[cfg(feature = "binary")]
impl BuildingBlueprint {
    /// Rebuild the tactical subset from an archived blueprint.
    #[must_use]
    pub fn from_archived(a: &crate::world::binary::archives::ArchivedBuildingBlueprint) -> Self {
        Self {
...
/// A `[f32; 2]` pair on the wire as the `f64` pair the parser holds.
#[cfg(feature = "binary")]
fn pair(p: &[rkyv::rend::f32_le; 2]) -> [f64; 2] {
    [f64::from(p[0].to_native()), f64::from(p[1].to_native())]
}

#[cfg(feature = "binary")]
fn level_from_archived(a: &crate::world::binary::archives::ArchivedBuildingLevel) -> BuildingLevel {
    BuildingLevel {
