...
# V1 Git history (read-tree snapshot commits + merge parents)
wc=$(git log --oneline website/ 2>/dev/null | wc -l || echo 0)
[[ "$wc" -ge 1 ]] && pass "V1 website/ has git history ($wc commits)" || fail "V1 website/ history"

wc=$(git log --oneline mod/ 2>/dev/null | wc -l || echo 0)
[[ "$wc" -ge 1 ]] && pass "V1 mod/ has git history ($wc commits)" || fail "V1 mod/ history"

# V2/V3 content parity — spot-check key paths exist
for p in website/cmd/api website/frontend mod/tbd-framework shared/tbd-schema tickets/registry.json; do
  [[ -e "$p" || -f "$p" ]] && pass "V2 path exists: $p" || fail "V2 missing: $p"
done

# V4 shared schema lift
[[ -d shared/tbd-schema ]] && pass "V4 shared/tbd-schema" || fail "V4 shared/tbd-schema"
...
nid=$(python3 -c "import json; print(json.load(open('tickets/registry.json'))['next_id'])")
[[ "$nid" -ge 121 ]] && pass "V6 next_id=$nid" || fail "V6 next_id"

# V7 registry at root
[[ -f tickets/registry.json ]] && pass "V7 tickets at root" || fail "V7 tickets"

# V8 scripts at root
[[ -x scripts/ticket ]] && pass "V8 scripts/ticket" || fail "V8 scripts/ticket"

# V9 artifacts
[[ -d artifacts ]] && pass "V9 artifacts/" || fail "V9 artifacts"

# V10 T-068 ready
python3 -c "
import json, sys
r=json.load(open('tickets/registry.json'))
...
# V12 README
[[ -f README.md ]] && grep -q 'website/' README.md && pass "V12 README" || fail "V12 README"
...
if (cd website/frontend && npm run build >/dev/null 2>&1 && npm run lint >/dev/null 2>&1); then
...
# V15 Go compile
(cd website && go build ./...) && pass "V15 go build" || fail "V15 go build"

# V16 test-it (optional if db down — warn only)
if (cd website && TEST_DATABASE_URL=postgres://tbd:tbd@localhost:5434/tbd_reforger?sslmode=disable go test ./internal/handlers/... >/dev/null 2>&1); then
  pass "V16 test-it"
else
...
# V17 schema validate
if (cd shared/tbd-schema && npm ci --silent >/dev/null 2>&1 && node scripts/validate.mjs >/dev/null 2>&1); then
...
if ! rg -q 'Tbdevent_Website' mod/scripts mod/docs 2>/dev/null; then
  pass "V18 no Tbdevent_Website in mod scripts/docs"
else
...
[[ -f website/CLAUDE.md ]] && grep -q 'Canonical context' website/CLAUDE.md && pass "V22 website stub" || fail "V22 website stub"
...
grep -q 'shared/tbd-schema' .github/workflows/schema.yml && pass "V25 schema CI" || fail "V25 schema CI"

# V26 specs not under website/Design_Docs
