...
impl PakVfs {
    ...
    /// Diagnostic: the DATA-chunk payload offset of the pak this path resolves into.
    ///
    /// T-305 — `data_start` is **not** part of any seek. Entry offsets are absolute from the
    /// pak start (T-206 measured 300/300 compressed entries inflating at `entry.offset` and
    /// only 3/300 at `data_start + offset`); adding it rotated every read by 56 bytes. The
    /// value is kept, and exposed here, purely to report where a pak's payload begins.
    pub fn entry_data_start(&self, virtual_path: &str) -> Option<u64> {
        self.index
...
/// T-305 — the synthetic-pak fixture that pins "entry offsets are absolute".
///
/// The pak is built byte-by-byte in memory so the fixture states the format claim outright:
/// `data_start` is 56 (the DATA payload offset of every shipped pak) and the two entries are
/// stored at **absolute** file positions 56 and 312. A reader that seeks `data_start + offset`
/// therefore lands 56 bytes late on every read, which is exactly what these tests catch.
#[cfg(test)]
mod tests {
    use super::*;
    use std::fs;
