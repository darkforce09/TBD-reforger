...
use crate::terrain::dem::grid::DEM_VECTOR_GRID_FACTOR;
use crate::terrain::dem::grid::DemVectorGrid;
use crate::terrain::dem::grid::downsample_dem_grid;
use crate::terrain::dem::grid::reduce_grid_2x;
use crate::terrain::relief::contours::contour_grid_reductions;
use crate::terrain::relief::contours::contour_levels;
use crate::terrain::relief::contours::contour_rings;
use crate::terrain::relief::contours::summit_ring_indices;
use crate::terrain::relief::sea_band::build_sea_band_geometry;
use crate::terrain::relief::sea_band::sea_fill_alpha;
...
impl DemVectors {
    ...
    fn push_sea(&mut self, engine: &EngineHandle, zoom: f64, grid: &DemVectorGrid) {
        ...
        if !class_visible("sea", zoom) || alpha <= 0.0 {
            if let Some(e) = engine.borrow_mut().as_mut() {
                e.clear_vector_lane(crate::core::pipeline::draw_order::role_id::SEA);
            }
            self.sea_built_alpha = -1.0;
            return;
        }
        ...
        if let Some(e) = engine.borrow_mut().as_mut() {
            e.upload_polygon_mesh(
                crate::core::pipeline::draw_order::role_id::SEA,
                &mesh.positions,
                &mesh.colors,
                &mesh.indices,
                mesh.polygon_count,
    ...
    fn push_contours(&mut self, engine: &EngineHandle, zoom: f64, grid: &DemVectorGrid) {
        if !class_visible("contour", zoom) {
            if let Some(e) = engine.borrow_mut().as_mut() {
                e.clear_vector_lane(crate::core::pipeline::draw_order::role_id::CONTOURS);
            }
            self.last_interval = 0.0;
            return;
        }
        ...
        if let Some(e) = engine.borrow_mut().as_mut() {
            e.upload_hairline_segments(
                crate::core::pipeline::draw_order::role_id::CONTOURS,
                &hair.verts,
                hair.segment_count,
                true,
            );
