...
use crate::spatial::world_los::tlas::AabbTlas;

/// One chunk row: engine-frame position (`[x, y_up, z_north]`), `GetAngles()` degrees (`[pitch, yaw, roll]`), uniform scale.
...
impl ChunkOccluder {
    /// Build from a parsed chunk. `bounds_of(pid)` yields the object-frame bounds a row's geometry currently has (`None` = no geometry, the row never crosses anything) and whether those bounds are a proxy.
    #[must_use]
    pub fn build(
        id: &str,
        chunk: &WorldChunk,
    ...
    /// Recompute every row's world box and the TLAS (after a descriptor expands or a BLAS is evicted).
