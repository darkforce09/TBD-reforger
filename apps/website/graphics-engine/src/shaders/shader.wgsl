//! Role: map vertex, fragment, and instance-culling programs.
//! Position: shared shaders consumed by the map render pipelines.
//! Signals & state: bound uniforms, vertex streams, textures, and storage buffers.
//! Invariants: Rust bindings, vertex strides, anchor coordinates, and clip depth must agree.
// Stream 0 is a unit quad
...
// Textured basemap and hillshade quads.
...
// World-building oriented-box fills.
...
// UV[N] + drag_delta + px_to_m (world glyphs: px_to_m=1, drag=0; slots: px size).
// N MUST equal Rust `ATLAS_GLYPH_COUNT` (32); the graphics shader-const test asserts
...
// ASCII text atlas: UV from glyph index, grid dimensions from the uniform.
...
// WebGPU instance culling and vertex/storage compaction.
...
// Workgroup-local visibility bits, exclusive prefix on thread 0, then one
// atomicAdd per workgroup (not per visible icon). The first workgroupBarrier must
// run before that reduction so every visibility bit is available.
