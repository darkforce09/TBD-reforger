...
//! Position: `symbology/instances/slots/tests` in the graphics engine.
...
fn yaw_encoders_agree() {
    for i in -400..=400 {
        ...
        assert_eq!(
            yaw_to_snorm16(deg),
            crate::symbology::labels::glyph_math::yaw_to_snorm16(deg),
    ...
    for deg in [0.0, 45.0, 90.0, 180.0, 359.5] {
        assert!(
            (screen_yaw_for_heading_deg(deg)
                - crate::symbology::labels::glyph_math::deck_angle_for_rotation_deg(deg))
            .abs()
                < 1e-12,
