...
impl IconComputeCull {
    ...
    /// The GPU counter value for stats: the real sampled count once available, else the CPU oracle mirror (flagged by [`Self::gpu_sampled`]). Summed across lanes.
    #[must_use]
    pub fn gpu_count_for_stats(&self) -> u32 {
        self.lanes.values().map(lane_gpu_count_for_stats).sum()
    }

    /// Lane gpu count for stats.
    #[must_use]
