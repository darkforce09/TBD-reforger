...
/// body-only PATCH must rederive `snippet` on write. Without this, the list view keeps
...
fn body_only_patch_rederives_snippet_on_write() {
    ...
    assert!(
        ...
        "body-only PATCH must gate rederive on snippet absence (fails with: always skip)"
    );
    assert!(
        handler.contains("snippet_from(\"\", b)"),
        "body-only PATCH must call snippet_from(\"\", b) to derive on write"
    );
    // Assembled so a comment naming the call cannot satisfy — the bind must sit next to the body arm.
...
/// the CMS list must be AdminUser-gated and filter drafts+published on both the count
/// and list SQL (not published-only; not a comment naming the filter).
///
/// Fails when:
...
fn list_cms_announcements_is_drafts_plus_published_not_public_feed() {
    ...
    let start = prod
        ...
        .expect("CMS GET list handler must exist (fails with: remove list_cms_announcements)");
    let after = &prod[start..];
    ...
    assert!(
        ...
        "list_cms_announcements must take `{admin_pin}` (fails with: remove AdminUser)"
    );
    ...
    assert_eq!(
        ...
        2,
        "count + list SQL must each use `{filter}` (a comment naming it, or published-only SQL, fails this)"
    );
    ...
    // pass behind a listing handler that still compiles.
