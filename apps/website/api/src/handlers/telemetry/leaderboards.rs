...
mod tests {
    ...
    //! nothing off the whitelist ever reaches ORDER BY. Both are pure string pins.
    //!
    //! The defect's DB-observable half — LIMIT/OFFSET paging over the T-194 content golden's
    //! 4-way ties yielding every row exactly once, in the one order the whitelist specifies — is
    //! `tests/leaderboards_paging.rs`. It used to live here, provisioning its own database from a
    //! raw `TEST_DATABASE_URL` read; the T-542/T-558 Class-R pin
    //! (`tests/common/mod.rs`, `t542_no_raw_test_database_url_reads_outside_common`) forbids
    //! that anywhere under `src/**` — only `common::require_test_database_url` may read the
    //! variable, and `tests/common` is not reachable from a lib test.
