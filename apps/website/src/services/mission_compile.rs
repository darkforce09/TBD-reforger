...
use crate::contract::load_kit_aliases;
...
pub fn flatten_to_mod_document(
    ...
    let max_players = if m.max_players < 1 {
        (doc_slots.len() as i64).max(1)
    } else {
        m.max_players
    };

    let mut terrain = m.terrain.as_str().to_string();
    if terrain == "custom" && !m.custom_terrain_name.is_empty() {
        terrain = m.custom_terrain_name.clone();
    }

    let meta = ModMeta {
        id: mission_doc_id(&m.id.to_string()),
        name: if m.title.is_empty() {
