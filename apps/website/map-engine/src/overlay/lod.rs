//! Role: zoom gates — what is legible at this scale.
//! Position: `overlay` in the map engine.
//! Signals & state: a table of zoom thresholds per feature class, and the contour interval.
//! Invariants: these are cartographic legibility rules, not culling. Nothing here asks
//! whether a thing is on screen; it asks whether drawing it at this scale would help.
//!
//! T-0xx Phase 2B.1: from `core/culling/lod.rs`. It sat under `culling/` and culls nothing —
//! `class_visible`, `contour_interval_for_zoom`, `TARGET_SPACING_PX` and
