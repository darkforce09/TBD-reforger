//! Role: lane preferences — the string → lane cartography table the browser API speaks.
//! Position: `overlay` in the map engine.
//! Signals & state: which world layers are visible, and each lane's tint alpha.
//! Invariants: `set_world_layer_visible` maps a layer *name* to a lane, which makes it
//! cartography, not engine state. `set_lane_opacity` is the exception in the other direction:
//! it writes a tint at byte offset 16 of a live uniform buffer, so packet mutation rides
//! along in this file. That is why it moved whole rather than being split four methods wide —
//! splitting it would have put half a method's worth of context in two directories.
//!
//! T-0xx Phase 2B.1: from `core/context/preferences.rs`.
...
impl RenderEngine {
    ...
    pub fn set_grid(&mut self, width: f64, height: f64, over_hillshade: bool, visible: bool) {
        let verts = crate::overlay::lanes_prefs::grid_lines(width, height, over_hillshade);
        if verts.is_empty() {
...
// preferences; the line arithmetic itself is `website_graphics_engine::draw::grid`.
#[must_use]
pub fn grid_lines(
    width: f64,
    height: f64,
    over_hillshade: bool,
) -> Vec<website_graphics_engine::draw::geometry::LineVertex> {
    website_graphics_engine::draw::grid::grid_lines(
