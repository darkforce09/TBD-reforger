//! Role: the 48 named lane identities, their paint order, and the two public id namespaces.
//! Position: `overlay` in the map engine.
//! Signals & state: names and a declaration order. No GPU handle, no buffer, no geometry.
//! Invariants: **declaration order IS paint order** — `ALL_LANES` and `lane_id` both key off
//! it, and moving a variant moves what draws on top of what. The `u32` values in `role_id` and
//! `tex_role_id` are wire ids the browser API passes in; they never change.
//!
//! T-0xx Phase 2B: this is `core/pipeline/{draw_order,roles}.rs` folded into one file and
