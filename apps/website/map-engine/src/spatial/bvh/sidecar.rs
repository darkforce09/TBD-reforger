...
/// The ONE authority for the determinism cast: quantize f64→f32 per component, then build the BVH over [`lift_verts`] of the result — never over the raw f64s.
...
/// Every way [`BvhSidecar::parse`] rejects bytes. The battery in `bvh_tests.rs` exercises one case per variant.
...
pub enum BvhParseError {
    ...
    /// Tri index out of bounds.
    TriIndexOutOfBounds { tri: u32 },

    /// Non finite node bound.
...
/// A parsed sidecar, ready to raycast: verts are pre-lifted to f64 so `sc.bvh.any_hit(&sc.verts, &sc.tris, ..)` is the whole call; `kinds` is parallel to `tris` (all `Opaque` for a version-1 file).
#[derive(Debug)]
...
impl BvhSidecar {
    /// An all-`Opaque` sidecar from parts already built over `verts` — the test scenes' and the pre-v2 callers' constructor. Panics when `bvh` was built over a different mesh.
    #[must_use]
...
impl BvhSidecar {
    /// Full structural validation — after `parse` succeeds, [`Bvh::any_hit`] can neither panic nor hang no matter what the bytes were. See the module doc for the format.
    pub fn parse(bytes: &[u8]) -> Result<BvhSidecar, BvhParseError> {
        use crate::spatial::bvh::sidecar::BvhParseError as E;
...
/// Serialize a version-2 sidecar. Infallible: malformed *bytes* only enter through [`BvhSidecar::parse`]; a mismatched input here is programmer error. Contract: `bvh` was built over `lift_verts(verts_f32)` and exactly these `tris`; `kinds` is parallel to `tris`.
pub fn emit_bytes(
    verts_f32: &[[f32; 3]],
    tris: &[[u32; 3]],
    kinds: &[SurfaceKind],
