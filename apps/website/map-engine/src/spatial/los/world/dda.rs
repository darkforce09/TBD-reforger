...
/// Cells `(cx, cy)` of `cell_m`-sized chunks the 2-D segment `a→b` crosses, first to last, endpoints inclusive, restricted to `0 ≤ cx < cols`, `0 ≤ cy < rows` (cells off the terrain are skipped, the walk continues). A zero-length segment yields its own cell.
#[must_use]
...
/// Brute-force reference: every cell whose square the segment touches, ordered by the parametric entry of the segment into the cell (ties by `(cx, cy)`).
#[must_use]
