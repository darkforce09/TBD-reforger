//! Role: styling.
//! Position: `terrain/roads` in the graphics engine.
//! Signals & state: camera, spatial, asset, or GPU data owned by this module.
//! Invariants: preserve coordinates, resource lifetimes, ordering, and binary layouts.

/// One triangle-list vertex: position + RGBA (normalized 0..1). Layout matches `lanes::LineVertex` / polygon fill (24 B) so the same `PolygonFill` pipeline can draw road strips.
...
/// Dash pattern [dash, gap] in meters (mirror `dashArrayFor`: dashed roads = [8, 6]). Emits only the dash intervals of the path as separate joined strips.
#[must_use]
...
/// Canonical runway casing rgba value.
...
/// Canonical runway polish width m value.
...
/// Pack strip vertices into a flat f32 buffer: `[x,y,r,g,b,a]…` (6 f32 per vertex) in **world** meters — the engine converts to anchor-relative on upload.
#[must_use]
