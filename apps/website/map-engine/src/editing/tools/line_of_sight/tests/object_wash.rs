//! Role: goldens for the coarse-to-fine object pass over a synthetic viewshed.
//! Position: `editing/tools/line_of_sight/tests` in the map engine.
//! Signals & state: a synthetic viewshed and a counting cell test built in the test body.
//! Invariants: blocks are tested nearest-first within a level, only over terrain-visible cells, and
//! an unreachable occluder makes the pass WAIT rather than mark a cell.
