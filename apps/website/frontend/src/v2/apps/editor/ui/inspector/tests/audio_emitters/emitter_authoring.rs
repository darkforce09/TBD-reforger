...
fn place_on_map_reuses_the_marker_gesture() {
    const ROOT: &str = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/v2/apps/editor/ui/inspector/audio_emitters.rs"
    ));
    const VIEW: &str = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/v2/apps/editor/ui/inspector/audio_emitters/view.rs"
    ));
    assert!(
        VIEW.contains("begin_place_marker"),
        "placement must call begin_place_marker, not a new gesture"
    );
    assert!(ROOT.contains(PLACE_MARKER_ICON));
    assert!(VIEW.contains("PLACE_MARKER_ICON"));
    assert!(VIEW.contains("arm_place_on_map"));
}
