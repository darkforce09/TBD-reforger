...
//! reaches it through `crate::v2::apps::editor::mission_editor::…` and it bridges the page to the strip,
...
thread_local! {
    ...
    ///
    /// `ArcRwSignal` (not `RwSignal`) for the reason [`crate::v2::apps::editor::mission_editor`]'s
...
/// A Z gesture snapshots authored f64 heights once, then previews without writing the document.
/// Keeping ids paired with heights avoids flattening a selection or exchanging neighbours' Zs.
#[derive(Clone, Debug)]
...
impl ZDrag {
    ...
    pub(crate) fn begin(
        ...
        for id in ids {
            // Raw rows retain f64 precision and include hidden entities omitted by the render SoA.
...
/// page but not `editor_ops` — can name it. `editor_ops` re-uses it via `crate::v2::apps::editor::mission_editor::…`.
