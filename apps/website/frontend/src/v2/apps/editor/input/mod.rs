...
//! [`super::state`]; nothing under [`super::panels`] routes events through here.
...
//! the layer. Undo and redo have exactly one entry point — `state::history` — and the keydown
...
/// and the undo/redo shortcuts that call into `state::history`.
#[cfg(target_arch = "wasm32")]
