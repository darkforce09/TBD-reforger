//! Stores the owner-scoped warm editor marker and purges obsolete adoption markers.
...
/// across missions wins), exactly as `editorSession.ts`. No longer a physical key (  see
/// [`session_key`]); kept as the scoped suffix, and to recognise what a pre- build left behind.
...
///  established for the IndexedDB records and  extended to the snapshot RAM cache.
///
/// **Why this one is scoped rather than deleted ().** The record holds
...
/// [`crate::v2::apps::editor::shell::persist::owner_token`] is `pub` () and is the one token every per-account cache has
/// to agree on.
///
/// Scoping alone closes the hole  B resolves a different physical key and cannot even name A's record —
/// so no sign-out purge is needed, which keeps this clear of the  ordering trap (a token resolved
...
/// `Date.now()` epoch-ms value; `currentSemver` is `null` until a server version is known.
...
pub fn mark_ready(mission_id: &str, slot_count: u32, current_semver: Option<String>) {
    ...
    if let (Some(storage), Ok(json)) = (
        ...
        // Drop the pre- unscoped record while we are here. `read_warm` reads the scoped key now,
...
pub fn clear() {
    if let Some(storage) = web_sys::window().and_then(|w| w.session_storage().ok().flatten()) {
        let _ = storage.remove_item(&session_key());
        let _ = storage.remove_item(SESSION_KEY); // and the pre- unscoped record
    }
}

/* ────────── adopted-server marker  REMOVED (; reader removed by ) ────────── */

/// The key prefix this module wrote until , kept
/// solely so [`purge_legacy_markers`] can still recognise the residue.
const ADOPTED_KEY_PREFIX: &str = "tbd-editor-adopted:";

/// Erase every `tbd-editor-adopted:*` key written before .
///
...
///
/// **Why this deletes rather than warns.** 's legacy-record path warns once and offers adoption
...
/// judgement call: this marker has **no reader** ( removed the last one;  confirmed zero
...
/// cross-account disclosure  and  spent two slices closing, and it would otherwise persist
...
/// during a deploy rollover a tab still running pre- wasm can write *fresh* residue at any
...
///
/// ** / this is now the only path to the residue, and it has exactly one caller.**
///  hung the purge off `mark_adopted` because that slice did not own the files the call sites
/// live in, and  then measured the consequence: three editor paths reach the document without
...
/// rather than guaranteed.  moved the call to `mission_hydrate::hydrate_from_server`, before
/// its `is_uuid` guard and before the fetch, so it runs on every editor boot on every branch;
