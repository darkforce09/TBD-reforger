//! Editor chrome dimensions, collapse state, and pane-center geometry.
//! Shared constants align dock CSS classes with camera insets and keep the map center stable.
...
// : the LIVE inset is `dock_left_px()` etc.; these consts are the expanded value the accessors
...
///
/// ** does NOT touch this.** The strip height is a separate contract from the dock widths:
///  split the strip into two rows that SUM to it ([`ROW_MENUS_PX`] + [`ROW_TOOLS_PX`]), and the
/// `top-12`/`h-12` utilities are written from it. Equalising the docks moves the X insets only.
pub const STRIP_TOP_PX: f64 = 48.0;

/// the EQUALISED dock width, in CSS px. Eden is 240/240 in every one of the 75 screenshots;
/// we were 256 left and 320 right, which is what pushed the right dock's trailing tab off the
/// viewport (the  clipping this ticket absorbed  the clipping was a symptom of the width).
///
...
/// Left dock width — [`DOCK_LEFT_CLASS`] (`width utility 60`). Expanded value; live inset is [`dock_left_px`]
/// (→ [`STUB_PX`] collapsed, → 0 while `chrome_hidden`).
pub const DOCK_LEFT_PX: f64 = DOCK_PX;
/// Right dock width — [`DOCK_RIGHT_CLASS`] (`width utility 60`). Expanded value; live inset is [`dock_right_px`]
/// (→ [`STUB_PX`] collapsed, → 0 while `chrome_hidden`).
...
///
///  split the single floating pill into a full-width status bar docked at `inset-x-0 bottom-0`
...
/// the collapsed dock stub: a 24×24 square in the panel's outer top corner (Eden, measured
...
/// accessors, the Tailwind stub size (`width utility 6 h-6`), and the 24×24 chevron hit-box.
pub const STUB_PX: f64 = 24.0;

/// the dock wrappers' BOTTOM inset in CSS px: how far above the viewport bottom an expanded
...
// Dock mount classes and numeric insets share one width contract. The camera uses the
// numeric insets to unproject clicks, and the DOM mounts these same dimensions.

/// the LEFT dock wrapper's classes while EXPANDED. The `w-*` token is the DOM half of
/// [`DOCK_LEFT_PX`]; `top-12` is the DOM half of [`STRIP_TOP_PX`]; 's `bottom-9` is the DOM half
...
/// the LEFT dock wrapper while COLLAPSED (): no `w-*`, no `bottom-*`, so the wrapper
...
/// the RIGHT dock wrapper while expanded. Same `width utility 60` as the left: that IS the equalisation.
///  `bottom-9` (= [`DOCK_BOTTOM_PX`]) matches the left: both docks stop at the status bar's top.
pub(crate) const DOCK_RIGHT_MOUNT: &str = "absolute bottom-9 right-0 top-12 z-20 w-60";
/// the RIGHT dock wrapper while collapsed. See [`DOCK_LEFT_MOUNT_COLLAPSED`].
pub(crate) const DOCK_RIGHT_MOUNT_COLLAPSED: &str = "absolute right-0 top-12 z-20";

/// the Tailwind v4 spacing scale, in CSS px: `width utility 60` / `h-4` / `size-6` → `N × 4.0`
...
/// the width a `w-*` token in `classes` resolves to in CSS px. See [`tw_len_px`].
#[must_use]
...
thread_local! {
    /// the left (Entity List) dock's collapse latch. Session-local (no prefs store: the
    /// `world_layer_prefs` seam is out of this ticket's owns  persisting collapse as an editor
    /// preference is left as residue for , per the ticket). Mirrored here from the reactive
    ...
    /// the right (Asset Browser) dock's collapse latch. See [`DOCK_LEFT_COLLAPSED`].
    static DOCK_RIGHT_COLLAPSED: Cell<bool> = const { Cell::new(false) };
    /// mirror of `mission_editor`'s  `chrome_hidden`. Hidden ⇒ every inset accessor
...
/// live RIGHT inset (CSS px). See [`dock_left_px`].
#[must_use]
...
/// live TOP inset (CSS px): the strip does not collapse, so this is [`STRIP_TOP_PX`] unless
...
/// live BOTTOM band (CSS px): the toolbelt band does not collapse, so this is
...
/// the live BOTTOM inset of a dock WRAPPER (CSS px): the DOM half of the mount classes, not
...
/// against the same const the DOM half is pinned to (the  class↔const discipline, on the Y axis).
#[must_use]
...
// ──  map-pane centre + camera centre-hold (pure; native-tested) ─────────────────────────────

/// the centre of the MAP PANE in screen (CSS) px for a `width × height` viewport, using the
...
/// CENTRE-HOLD decision, as pure camera math.
///
...
// ──  the top strip's shell, folded back from `eden_top_strip.rs` ──────────────────────────
//
//  needed a two-row strip shell and a tighter icon recipe. `eden_layout` was another
...
//     [`STRIP_ROWS`] at , which left `STRIP` referenced by nothing but the test that compared
...
/// the top strip's shell: [`OVERLAY_DOCKED`]'s glass, the
...
/// strip row 1, Eden's `y 0–22`: the menu bar, the editable title
...
/// strip row 2, Eden's `y 22–40`: history/undo/redo, the ORBAT
...
/// the menu row's fixed height (`h-6`). Documentation-with-teeth: the pins
/// check `ROW_MENUS_PX + ROW_TOOLS_PX == STRIP_TOP_PX`, so the split can never grow the strip.
pub(crate) const ROW_MENUS_PX: f64 = 24.0;
/// the tool row's height: the remainder, 48 − 24. Not a class value.
pub(crate) const ROW_TOOLS_PX: f64 = 24.0;

/// `cn(overlayDocked, …)` + the dock's own edge border.
///
/// the padding drops `p-3` → `p-2`. At 320 px the right dock could afford 12 px of gutter on
...
/// The shared icon-button recipe (React TopCommandStrip:148), as  rebuilt it.
///
...
/// row or a dense dock.  could not fix it here (this file was another slice's owns that wave),
/// so it routed around the defect with a local `TOOL_ICON` copy and left the defect standing for
/// every OTHER caller.  fixes the recipe and deletes the copy.
///
/// A live control now rests at full `text-on-surface`; dimming is reserved for DISABLED, where it
/// means exactly one thing. State comes from the  vocabulary ([`HOVER_FILL`] +
...
// ──  the one state vocabulary (editor_chrome_direction.md §"The state vocabulary") ──────────
//
