...
//!
//! **Role:** defines the four ranked tiers, the snake_case spelling they travel in, parsing of a
...
//! **Invariants:** the ladder is total and ordered `Enlisted < Leader < MissionMaker < Admin`.
...
//!
//! A gate sees **five** outcomes, not four: "no tier at all" (`None` — a guest, or a session that
...
//! | Tier | Reaches |
//! |---|---|
//! | `None` (guest / pre-bootstrap) | every open route and, in browse mode, the whole sidebar; no affordance that acts on data |
...
#[allow(dead_code)]
pub enum Role {
    /// A signed-in member: the baseline tier every authenticated account holds.
    Enlisted,
...
impl Role {
    /// This tier's position on the ladder, `1` for the lowest. Only the ordering is meaningful.
    fn rank(self) -> u8 {
        match self {
            Role::Enlisted => 1,
            Role::Leader => 2,
    ...
    /// Parse the tier a route declares. An open route spells this `"none"`; anything unrecognised
    /// is treated the same way, so a typo opens a route rather than locking everyone out.
...
/// tool would flash as authorised for a guest, and for the moment between mount and the session
...
///
/// The mirror image of [`has_min_role`] on the `None` case: a guest, or a session that has not
