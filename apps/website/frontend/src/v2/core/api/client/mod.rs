...
//! still `401`, propagates to the caller. The request verbs are browser-only, while the failure
//! types and the refresh policy compile natively so the policy can be tested without a browser.
