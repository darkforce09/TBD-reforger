...
pub fn begin_layer_drag(layer_id: String) {
    website_map_engine::data::store::operations::entity::begin_layer_drag(layer_id);
}
...
pub fn begin_layer_slot_drag(slot_id: String) {
    website_map_engine::data::store::operations::entity::begin_layer_slot_drag(slot_id);
}

/// Begin layer comment drag using the supplied domain data.
pub fn begin_layer_comment_drag(comment_id: String) {
    website_map_engine::data::store::operations::entity::begin_layer_comment_drag(comment_id);
}

/// Drop an armed drag ANYWHERE that isn't a valid target (clear without mutating).
pub fn cancel_layer_drag() {
    website_map_engine::data::store::operations::entity::cancel_layer_drag();
}
...
pub fn complete_layer_drop_onto_folder(dest_folder_id: String) -> bool {
    let dropped = OPS_CTX
        .with(|c| {
            ...
            let core = d.as_ref()?;
            Some(
                website_map_engine::data::store::operations::entity::complete_layer_drop_onto_folder(
                    core,
                    &dest_folder_id,
                ),
            )
...
pub fn complete_layer_drop_onto_root() -> bool {
    let dropped = OPS_CTX
        .with(|c| {
            ...
            let core = d.as_ref()?;
            Some(
                website_map_engine::data::store::operations::entity::complete_layer_drop_onto_root(
                    core,
                ),
            )
        })
