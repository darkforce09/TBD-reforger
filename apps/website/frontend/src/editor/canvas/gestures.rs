...
fn z_drag_snap_step(snap: RwSignal<transform::SnapState>) -> f64 {
    ...
    let onpointermove = Closure::<dyn FnMut(web_sys::PointerEvent)>::new({
        ...
        move |ev: web_sys::PointerEvent| {
            ...
            if let Some((start_y, slot_ids, _veh_ids, initial_zs, cam)) = z_arm {
                let delta =
                    z_drag_elevation_delta(py, start_y, cam.scale(), z_drag_snap_step(snap));
...
//    production item in this file must stay above this line — and nothing below it is visible to
//    the pins in `mission_editor_tests/` that scrub this file. (This trap made T-937.4's own pin
//    dead in wave 253 and T-937.1's in 252.)
#[cfg(test)]
mod t946_86_z_arm {
    use crate::editor::arsenal::class_r_scrub::live_code;
    use crate::editor::canvas::gestures::z_drag_elevation_delta;

    /// This file's LIVE source — comments stripped, string/char literals blanked, test modules
    ...
    fn the_preview_and_the_commit_share_one_arithmetic() {
        ...
        assert_eq!(
            calls, 3,
            "T-946.86 (.82): expected the definition plus exactly two call sites (preview + \
             commit) — found {calls}. A third would be a second vocabulary; one means a call site \
             was lost"
        );
    }
