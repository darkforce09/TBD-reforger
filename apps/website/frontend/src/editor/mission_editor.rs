...
pub fn MissionEditorPage() -> impl IntoView {
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        canvas_ref.on_load(move |canvas: web_sys::HtmlCanvasElement| {
            ...
            // keeps using (keydown, pointercancel, pointerleave, the boot tasks all still read
            // them), and carries the `Copy` signals by value exactly as the closures captured them
            // here. `attach_canvas_gestures` registers on the same container with the same
