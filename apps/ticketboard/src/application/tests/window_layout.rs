...
fn board_state(selected: Option<usize>) -> WorkspaceState {
    let corpus = corpus_of(vec![
        work("T-1", "status = \"idea\"", ""),
        work("T-2", "status = \"idea\"", ""),
    ]);
    let mut b = WorkspaceState::new(
        corpus,
        LockState::Missing {
...
/// Back contract under the beside-layout, pinned on the
...
/// The persisted viewer width is revalidated on load: parseable
