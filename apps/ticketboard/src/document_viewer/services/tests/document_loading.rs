use super::*;
use crate::test_support::{Scratch, work};
...
fn state_machine_transitions() {
    ...
    s.land(LoadedDocument {
        rel: "docs/a.md".into(),
        outcome: DocumentOutcome::Rendered {
            text: "# hi".into(),
        },
    });
    assert_eq!(
    ...
    s.land(LoadedDocument {
        rel: "docs/b.md".into(),
        outcome: DocumentOutcome::Fallback {
            text: String::new(),
...
fn stale_results_are_dropped() {
    let rendered = |rel: &str| LoadedDocument {
        rel: rel.into(),
        outcome: DocumentOutcome::Rendered {
            text: "stale".into(),
        },
...
fn classify_within_cap_renders() {
    ...
    assert_eq!(
        ...
        DocumentOutcome::Rendered { text: at_cap }
    );
}
...
fn classify_oversize_truncates_with_notice() {
    ...
    let DocumentOutcome::Fallback { text, note } = classify(bytes, 40, 16) else {
...
fn classify_oversize_cut_is_char_safe() {
    ...
    let DocumentOutcome::Fallback { text, note } = classify(bytes, 7, 2) else {
        panic!("oversize must fall back");
    };
...
fn classify_non_utf8_goes_lossy() {
    let DocumentOutcome::Fallback { text, note } = classify(vec![0x66, 0xFF, 0x66], 3, 1024) else {
    ...
    let DocumentOutcome::Fallback { note, .. } = classify(big, 12, 4) else {
...
fn classify_within_cap_tail_corruption_is_lossy() {
    ...
    let DocumentOutcome::Fallback { note, .. } = classify(bytes, 4, 1024) else {
...
fn load_doc_missing_file_names_the_error() {
    ...
    let DocumentOutcome::Fallback { text, note } = load_doc(s.path(), "docs/nope.md") else {
...
fn load_doc_refuses_symlink_escape() {
    ...
    let DocumentOutcome::Fallback { text, note } = load_doc(&root, "docs/link.md") else {
...
fn load_doc_capped_truncates_on_disk_file() {
    ...
    let DocumentOutcome::Fallback { text, note } = load_doc_capped(s.path(), "big.md", 4) else {
        panic!("oversize must fall back");
    };
