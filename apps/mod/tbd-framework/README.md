...
- Rules: `addon.gproj` names the vanilla data addon as its only dependency, and the addon carries
  no `Scripts/WorkbenchGame/` (`cargo xtask mod compile`); no upstream reference code or upstream-only asset GUID enters it
  (`cargo xtask verify no-crf-leak`); sources stay ASCII; every resource is committed with its
  `.meta`, and `resourceDatabase.rdb` is regenerated by Workbench, never edited.
