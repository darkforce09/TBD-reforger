...
{
	protected static const ResourceName LAYOUT_PATH = "{7BD1A70000000B01}UI/layouts/TBD_MissionSelector.layout";
	protected static const ResourceName TERRAIN_ROW_LAYOUT = "{7BD1A70000000B20}UI/layouts/TBD_TerrainRow.layout";
	protected static const ResourceName MISSION_CARD_LAYOUT = "{7BD1A70000000B40}UI/layouts/TBD_MissionCard.layout";
	...
	static void Open()
	{
		...
		if (!root)
		{
			// Fallback: direct bare path creation
			root = workspace.CreateWidgets("UI/layouts/TBD_MissionSelector.layout", null);
		}
	...
	protected void PopulateTerrains()
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			if (!rowWidget)
				rowWidget = ws.CreateWidgets("UI/layouts/TBD_TerrainRow.layout", m_wTerrainListContent);
	...
	protected void PopulateMissions()
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			if (!cardWidget)
				cardWidget = ws.CreateWidgets("UI/layouts/TBD_MissionCard.layout", m_wMissionListContent);
