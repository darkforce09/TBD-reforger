...
class TBD_MapExportPonds
{
	...
	protected static const float MAX_POND_AREA_M2 = 15000.0; // Ponds <= 15,000 m²; larger bodies are Lakes
	...
	bool Export(TBD_MapExportContext ctx, TBD_MapExportConfig cfg, out array<ref TBD_PondRecord> outPonds = null)
	{
		...
		Print(string.Format("%1 Pond export complete — Ponds=%2 (total area: %3 m²) -> %4",
	...
	protected void ExtractPonds(TBD_MapExportContext ctx, TBD_MapExportConfig cfg)
	{
		...
		for (int c = 0; c < candidateEntities.Count(); c++)
		{
			...
			Print(TAG + string.Format(" Exported Pond [%1]: '%2' (%3) — Area=%4 m², Depth(max/avg)=%5/%6 m, SurfaceY=%7 m, SplinePts=%8",
