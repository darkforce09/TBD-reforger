/**
 * TBD_MapExportFootpaths.c
 *
 * Dedicated Footpath, Walking Trail & Hiking Path extraction engine for Bohemia Reforger.
 * Queries world entities across spatial cells (512m), classifies pedestrian paths and hiking trails,
 * performs spatial spline discovery, stitches waypoints into continuous multi-point 3D curves (up to 150m step),
 * establishes topological graph connectivity, and stream-writes a valid JSON document (paths.json).
 *
 * Outputs:
 *   - $profile:TBD_Export/<mapName>/roads/paths.json
 */

class TBD_FootpathRecord
{
	int m_iId;
	string m_sId;
	string m_sName;
	...
	void TBD_FootpathRecord(int id, string name, float widthM, string prefab, string mat = "")
	{
		m_iId = id;
		m_sId = "path_" + id.ToString();
		m_sName = name;
		m_sRoadClass = "path";
		m_fWidthM = widthM;
		m_fTotalLengthM = 0.0;
...
class TBD_FootpathRawWaypoint
{
	...
	void TBD_FootpathRawWaypoint(vector pos, float widthM, string prefab, string mat)
	{
		m_vPos = pos;
		m_fWidthM = widthM;
		m_sPrefab = prefab;
...
class TBD_MapExportFootpaths
{
	protected static const string TAG = "[TBD][Roads][Paths]";
	protected static const float Y_MIN = -500.0;
	protected static const float Y_MAX = 1500.0;
	protected static const int FLUSH = 8000;
	protected static const float MAX_CHAIN_STEP_M = 150.0;

	protected ref array<IEntity> m_aHits;
	protected ref array<IEntity> m_aSpatialHits;
	protected ref array<ref TBD_FootpathRecord> m_aRecords;
	protected ref array<ref TBD_FootpathRawWaypoint> m_aRawWaypoints;
	...
	//------------------------------------------------------------------------------------------------
	//! Strict Footpath classifier.
	static bool IsFootpathEntity(string resName, string className, IEntity ent, IEntitySource src, out float widthM, out string roadName, out string matName)
	{
		widthM = 1.75;
		roadName = "";
		matName = "dirt_tread";
		...
		if (lowerMat.Contains("road_asphalt") || lowerMat.Contains("cobblestone") || lowerMat.Contains("road_dirt_01"))
			return false;
		if (lowerRes.Contains("highway") || lowerRes.Contains("mainroad") || lowerRes.Contains("asphalt") || lowerRes.Contains("paved") || lowerRes.Contains("cobble"))
			return false;
		if (lowerRes.Contains("road_dirt") || lowerRes.Contains("runway") || lowerRes.Contains("airstrip"))
			return false;
		...
		if (lowerMat.Contains("traildirt") || lowerMat.Contains("trailgravel") || lowerMat.Contains("trailforest") || lowerMat.Contains("trail_"))
			isPath = true;
		else if (lowerRes.Contains("path") || lowerRes.Contains("trail") || lowerRes.Contains("footpath") || lowerRes.Contains("pedestrian") || lowerRes.Contains("walkway") || lowerRes.Contains("hiking") || lowerRes.Contains("steps_stone"))
			isPath = true;
	...
	//------------------------------------------------------------------------------------------------
	//! Primary footpaths export execution method.
	bool Export(TBD_MapExportContext ctx, TBD_MapExportConfig cfg, out array<ref TBD_FootpathRecord> outRecords = null)
	{
		...
		string outJson = TBD_MapExportPaths.BuildCategoryPath(cfg.m_sDestinationDir, mapName, "roads", "paths.json");

		Print(string.Format("%1 Scanning footpaths & trails for '%2' (%3x%3 cells @ %4 m)...",
			TAG, mapName, cells, cellM), LogLevel.NORMAL);

		m_aRecords = new array<ref TBD_FootpathRecord>();
		m_aRawWaypoints = new array<ref TBD_FootpathRawWaypoint>();

		// 1. Gather all authentic footpath waypoints and discover attached splines
		for (int iz = 0; iz < cells; iz++)
		{
			for (int ix = 0; ix < cells; ix++)
			{
				...
				foreach (IEntity e : m_aHits)
				{
					...
					if (!IsFootpathEntity(rn, clsName, e, src, widthM, roadName, matName))
						continue;
					...
					if (matchedSpline)
					{
						...
						TBD_FootpathRecord splineRec = new TBD_FootpathRecord(recId, "Path_Spline_" + recId.ToString(), widthM, rn, matName);
					...
					{
						m_aRawWaypoints.Insert(new TBD_FootpathRawWaypoint(pos, widthM, rn, matName));
					}
				}
			}
		}
		...
		{
			TBD_FootpathRecord rec = m_aRecords[r];
		...
		Print(string.Format("%1 Extracted %2 continuous footpath segments (Total length: %3 m). Writing -> %4",
		...
		{
			TBD_FootpathRecord fpr = m_aRecords[i];
			buf += "    {\n";
			buf += "      \"id\": \"" + TBD_MapExportJson.Escape(fpr.m_sId) + "\",\n";
			buf += "      \"name\": \"" + TBD_MapExportJson.Escape(fpr.m_sName) + "\",\n";
			buf += "      \"roadClass\": \"" + TBD_MapExportJson.Escape(fpr.m_sRoadClass) + "\",\n";
			buf += "      \"widthM\": " + fpr.m_fWidthM.ToString() + ",\n";
			buf += "      \"totalLengthM\": " + fpr.m_fTotalLengthM.ToString() + ",\n";
			buf += "      \"pointsCount\": " + fpr.m_aPoints.Count().ToString() + ",\n";
			buf += "      \"points\": [\n";

			for (int pt = 0; pt < fpr.m_aPoints.Count(); pt++)
			{
				vector p = fpr.m_aPoints[pt];
				buf += "        [" + p[0].ToString() + ", " + p[1].ToString() + ", " + p[2].ToString() + "]";
				if (pt < fpr.m_aPoints.Count() - 1) buf += ",";
				buf += "\n";
			}
			buf += "      ],\n";

			buf += "      \"bounds\": {\n";
			buf += "        \"min\": [" + fpr.m_vBoundsMin[0].ToString() + ", " + fpr.m_vBoundsMin[1].ToString() + ", " + fpr.m_vBoundsMin[2].ToString() + "],\n";
			buf += "        \"max\": [" + fpr.m_vBoundsMax[0].ToString() + ", " + fpr.m_vBoundsMax[1].ToString() + ", " + fpr.m_vBoundsMax[2].ToString() + "]\n";
			buf += "      },\n";

			// Graph Connectivity
			buf += "      \"startNode\": {\n";
			buf += "        \"nodeId\": \"" + TBD_MapExportJson.Escape(fpr.m_sStartNodeId) + "\",\n";
			buf += "        \"pos\": [" + fpr.m_vStartNodePos[0].ToString() + ", " + fpr.m_vStartNodePos[1].ToString() + ", " + fpr.m_vStartNodePos[2].ToString() + "],\n";
			buf += "        \"connectedSegmentIds\": [";
			for (int s0 = 0; s0 < fpr.m_aStartConnectedSegments.Count(); s0++)
			{
				buf += "\"" + TBD_MapExportJson.Escape(fpr.m_aStartConnectedSegments[s0]) + "\"";
				if (s0 < fpr.m_aStartConnectedSegments.Count() - 1) buf += ", ";
			}
			buf += "]\n";
			buf += "      },\n";

			buf += "      \"endNode\": {\n";
			buf += "        \"nodeId\": \"" + TBD_MapExportJson.Escape(fpr.m_sEndNodeId) + "\",\n";
			buf += "        \"pos\": [" + fpr.m_vEndNodePos[0].ToString() + ", " + fpr.m_vEndNodePos[1].ToString() + ", " + fpr.m_vEndNodePos[2].ToString() + "],\n";
			buf += "        \"connectedSegmentIds\": [";
			for (int s1 = 0; s1 < fpr.m_aEndConnectedSegments.Count(); s1++)
			{
				buf += "\"" + TBD_MapExportJson.Escape(fpr.m_aEndConnectedSegments[s1]) + "\"";
				if (s1 < fpr.m_aEndConnectedSegments.Count() - 1) buf += ", ";
			}
			buf += "]\n";
			buf += "      },\n";

			buf += "      \"connectedSegmentIds\": [";
			for (int sc = 0; sc < fpr.m_aConnectedSegments.Count(); sc++)
			{
				buf += "\"" + TBD_MapExportJson.Escape(fpr.m_aConnectedSegments[sc]) + "\"";
				if (sc < fpr.m_aConnectedSegments.Count() - 1) buf += ", ";
			}
			buf += "],\n";

			buf += "      \"prefab\": \"" + TBD_MapExportJson.Escape(fpr.m_sPrefab) + "\",\n";
			buf += "      \"material\": \"" + TBD_MapExportJson.Escape(fpr.m_sMaterial) + "\"\n";
			buf += "    }";
		...
		Print(string.Format("%1 FOOTPATHS EXPORT FINISHED in %2 ms (Total=%3 continuous routes, %4 m) -> %5",
	...
	//------------------------------------------------------------------------------------------------
	//! Chains unvisited waypoints into continuous polyline ribbons (up to 150m radius).
	protected void ChainWaypointsIntoPolylines()
	{
		...
		{
			TBD_FootpathRawWaypoint curWp = m_aRawWaypoints[i];
			if (curWp.m_bVisited) continue;

			int recId = m_aRecords.Count() + 1;
			TBD_FootpathRecord rec = new TBD_FootpathRecord(recId, "Path_" + recId.ToString(), curWp.m_fWidthM, curWp.m_sPrefab, curWp.m_sMaterial);
			...
			while (foundNext)
			{
				...
				{
					TBD_FootpathRawWaypoint cand = m_aRawWaypoints[j];
				...
				if (bestIdx != -1)
				{
					TBD_FootpathRawWaypoint nextWp = m_aRawWaypoints[bestIdx];
	...
	protected void BuildConnectivity(array<ref TBD_FootpathRecord> records)
	{
		if (!records) return;
		...
		{
			TBD_FootpathRecord a = records[i];
			...
			{
				TBD_FootpathRecord b = records[j];
