...
class TBD_AttachmentScanner
{
	...
	protected bool IsOpticPrefab(string lowerPath, map<string, ref array<BaseContainer>> comps)
	{
		...
		if (lowerPath.Contains("collim_") || lowerPath.Contains("scope_"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SCR_OpticComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SCR_2DOpticComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "OpticComponent"))
			return true;
		return false;
	}
	...
	protected bool IsUnderbarrelLauncher(string lowerPath, map<string, ref array<BaseContainer>> comps)
	{
		if (lowerPath.Contains("/underbarrel/"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "WeaponComponent") && TBD_EquipmentComponentGraph.HasCompSuffix(comps, "MuzzleInMagComponent"))
			return true;
		return false;
	}
	...
	protected bool HasAttachmentComponent(map<string, ref array<BaseContainer>> comps)
	{
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SuppressorComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "BipodComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SCR_BayonetComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SCR_BayonetEffectComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "SCR_FlashlightComponent"))
			return true;
		if (TBD_EquipmentComponentGraph.HasCompSuffix(comps, "AttachmentSlotComponent"))
			return true;
		return false;
	}
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		if (anc)
		{
			string ancRes = TBD_EquipmentResourceNames.NormalizePathSeparators(anc.GetResourceName());
