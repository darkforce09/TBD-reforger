...
class TBD_MuzzleScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_MuzzleExtractor.CollectComponentChain(root, comps);

		// Complete weapons have WeaponComponent, they are not attachments
		if (TBD_MuzzleExtractor.HasCompSuffix(comps, "WeaponComponent") || TBD_MuzzleExtractor.HasCompSuffix(comps, "BaseWeaponComponent"))
			return;
		...
		bool hasSuppressorComp = TBD_MuzzleExtractor.HasCompSuffix(comps, "SuppressorComponent");
		...
		string canonical = TBD_MuzzleExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sFilePath = TBD_MuzzleExtractor.ResolveCanonical(path);
		...
		if (ancestor)
		{
			string ancRn = TBD_MuzzleExtractor.ResolveCanonical(ancestor.GetResourceName());
			if (!ancRn.IsEmpty() && ancRn != canonical)
				info.m_sVariantOf = ancRn;
		}

		// Raw string extraction (zero mutation, zero fake fallbacks)
		info.m_sDisplayName = TBD_MuzzleExtractor.RawDisplayNameFor(comps);
		info.m_sDescription = TBD_MuzzleExtractor.RawDescriptionFor(comps);
		info.m_sIcon = TBD_MuzzleExtractor.RawIconFor(comps);
