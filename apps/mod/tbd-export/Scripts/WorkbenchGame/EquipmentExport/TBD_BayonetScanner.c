...
class TBD_BayonetScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_BayonetExtractor.CollectComponentChain(root, comps);

		// Complete weapons have WeaponComponent, they are not attachments
		if (TBD_BayonetExtractor.HasCompSuffix(comps, "WeaponComponent") || TBD_BayonetExtractor.HasCompSuffix(comps, "BaseWeaponComponent"))
			return;
		...
		bool hasBayonetComponent = TBD_BayonetExtractor.HasCompSuffix(comps, "SCR_BayonetComponent") || TBD_BayonetExtractor.HasCompSuffix(comps, "SCR_BayonetEffectComponent");
		...
		string canonical = TBD_BayonetExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sFilePath = TBD_BayonetExtractor.ResolveCanonical(path);
		...
		if (ancestor)
		{
			string ancRn = TBD_BayonetExtractor.ResolveCanonical(ancestor.GetResourceName());
			if (!ancRn.IsEmpty() && ancRn != canonical)
				info.m_sVariantOf = ancRn;
		}

		// Raw string extraction (zero mutation, zero fake fallbacks)
		info.m_sDisplayName = TBD_BayonetExtractor.RawDisplayNameFor(comps);
		info.m_sDescription = TBD_BayonetExtractor.RawDescriptionFor(comps);
		info.m_sIcon = TBD_BayonetExtractor.RawIconFor(comps);
