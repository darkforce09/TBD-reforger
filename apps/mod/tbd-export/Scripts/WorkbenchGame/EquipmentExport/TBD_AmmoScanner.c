...
class TBD_AmmoScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_AmmoExtractor.CollectComponentChain(root, comps);

		bool hasMag = TBD_AmmoExtractor.HasCompSuffix(comps, "MagazineComponent") || TBD_AmmoExtractor.HasCompSuffix(comps, "BaseMagazineComponent");
		bool hasMove = TBD_AmmoExtractor.HasCompSuffix(comps, "MoveComponent")
			|| TBD_AmmoExtractor.HasCompSuffix(comps, "ShellMoveComponent")
			|| TBD_AmmoExtractor.HasCompSuffix(comps, "ProjectileMoveComponent")
			|| TBD_AmmoExtractor.HasCompSuffix(comps, "MissileMoveComponent")
			|| TBD_AmmoExtractor.HasCompSuffix(comps, "GrenadeMoveComponent")
			|| TBD_AmmoExtractor.HasCompSuffix(comps, "FlareMoveComponent");
		bool isProj = (rootClass == "Projectile" || rootClass == "TracerProjectile" || rootClass.EndsWith("Projectile"));

		if (!hasMag && !hasMove && !isProj)
			return;

		string canonical = TBD_AmmoExtractor.ResolveCanonical(root.GetResourceName());
	...
	protected void ProcessMagazine(string canonical, string path, BaseContainer root, map<string, ref array<BaseContainer>> comps, bool isAbstract)
	{
		...
		mag.m_sId = TBD_AmmoExtractor.GenerateSlug(path);
		...
		if (anc)
		{
			string ancRn = TBD_AmmoExtractor.ResolveCanonical(anc.GetResourceName());
	...
	protected void ProcessProjectile(string canonical, string path, BaseContainer root, map<string, ref array<BaseContainer>> comps, bool isAbstract)
	{
		...
		proj.m_sId = TBD_AmmoExtractor.GenerateSlug(path);
