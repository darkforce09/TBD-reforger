...
class TBD_StockScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_StockExtractor.CollectComponentChain(root, comps);

		// Complete weapons have WeaponComponent, they are not attachments
		if (TBD_StockExtractor.HasCompSuffix(comps, "WeaponComponent") || TBD_StockExtractor.HasCompSuffix(comps, "BaseWeaponComponent"))
			return;
		...
		string canonical = TBD_StockExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sFilePath = TBD_StockExtractor.ResolveCanonical(path);
		...
		if (ancestor)
		{
			string ancRn = TBD_StockExtractor.ResolveCanonical(ancestor.GetResourceName());
