...
class TBD_OpticScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_OpticExtractor.CollectComponentChain(root, comps);

		// Complete weapons have WeaponComponent, they are not attachments
		if (TBD_OpticExtractor.HasCompSuffix(comps, "WeaponComponent") || TBD_OpticExtractor.HasCompSuffix(comps, "BaseWeaponComponent"))
			return;
		...
		string canonical = TBD_OpticExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sFilePath = TBD_OpticExtractor.ResolveCanonical(path);
		...
		if (ancestor)
		{
			string ancRn = TBD_OpticExtractor.ResolveCanonical(ancestor.GetResourceName());
			if (!ancRn.IsEmpty() && ancRn != canonical)
				info.m_sVariantOf = ancRn;
		}

		// Raw string extraction (zero mutation, zero fake fallbacks)
		info.m_sDisplayName = TBD_OpticExtractor.RawDisplayNameFor(comps);
		info.m_sDescription = TBD_OpticExtractor.RawDescriptionFor(comps);
		info.m_sIcon = TBD_OpticExtractor.RawIconFor(comps);
