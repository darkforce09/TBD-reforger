...
class TBD_UnderbarrelScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_UnderbarrelExtractor.CollectComponentChain(root, comps);
		...
		if (!isUnderbarrel && isUnderbarrelPath && (isAbstract || TBD_UnderbarrelExtractor.HasCompSuffix(comps, "MuzzleComponent")))
			isUnderbarrel = true;

		if (!isUnderbarrel)
			return;

		string canonical = TBD_UnderbarrelExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sFilePath = TBD_UnderbarrelExtractor.ResolveCanonical(path);
		...
		if (ancestor)
		{
			string ancRn = TBD_UnderbarrelExtractor.ResolveCanonical(ancestor.GetResourceName());
			if (!ancRn.IsEmpty() && ancRn != canonical)
				info.m_sVariantOf = ancRn;
		}

		// Raw string extraction (zero mutation, zero fake fallbacks)
		info.m_sDisplayName = TBD_UnderbarrelExtractor.RawDisplayNameFor(comps);
		info.m_sDescription = TBD_UnderbarrelExtractor.RawDescriptionFor(comps);
		info.m_sIcon = TBD_UnderbarrelExtractor.RawIconFor(comps);
