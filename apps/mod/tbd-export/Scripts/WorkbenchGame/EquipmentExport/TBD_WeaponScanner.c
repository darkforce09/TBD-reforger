...
class TBD_WeaponScanner
{
	...
	protected void OnResourceFound(ResourceName resName, string filePath = "")
	{
		...
		TBD_WeaponExtractor.CollectComponentChain(root, comps);

		// Must be a weapon entity or explosive device
		bool isWeapon = TBD_WeaponExtractor.HasCompSuffix(comps, "WeaponComponent")
			|| TBD_WeaponExtractor.HasCompSuffix(comps, "MineWeaponComponent")
			|| TBD_WeaponExtractor.HasCompSuffix(comps, "MuzzleInMagComponent")
			|| TBD_WeaponExtractor.HasCompSuffix(comps, "MuzzleComponent")
			|| TBD_WeaponExtractor.HasCompSuffix(comps, "GrenadeMoveComponent");

		if (!isWeapon)
			return;

		string canonical = TBD_WeaponExtractor.ResolveCanonical(root.GetResourceName());
		...
		info.m_sId = TBD_WeaponExtractor.GenerateSlug(path);
		...
		if (anc)
		{
			string ancRn = TBD_WeaponExtractor.ResolveCanonical(anc.GetResourceName());
