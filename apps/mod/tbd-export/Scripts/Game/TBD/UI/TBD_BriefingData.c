...
//! One role line inside a group: "RFL x4", flagged when the reader's own seat sits on it.
...
//! it and no oracle documents it (SLICE_WORKFLOW.md SWhat agents cannot do). Until this slice the
...
class TBD_BriefingService
{
	...
	//! `"cafe latte".Substring(0, 4)` returns `caf` plus the FIRST BYTE of `e` - a broken UTF-8
