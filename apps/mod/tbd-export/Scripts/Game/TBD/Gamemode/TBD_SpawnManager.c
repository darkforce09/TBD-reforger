...
class TBD_DepartedSeat
{
	...
	//! S2 locks that choice for the synthesized case: a same-name reconnect keeps its spent life.
	bool reclaimable;
}
...
class TBD_SpawnManager : SCR_BaseGameModeComponent
{
	...
	//! which is wave/ticket respawn. See docs/mod/TBD_MOD_DESIGN.md S2 - this is a
	...
	//! T-541 - loadout settle poll. Wear verify alone is VERIFY_MAX_ATTEMPTS x VERIFY_TICK_MS
	//! (6 x 500 ms = 3 s) plus weapon phase (<=1 s) plus cargo/audit; 40 x 250 ms = 10 s is a
	...
	//! them: good enough to recognise a same-name reconnect (TBD_MOD_DESIGN.md S2 chooses exactly
	...
	//! TBD_MOD_DESIGN.md S2) permanently swallowed every mode-3 warning (numeric lease, ONE LIFE
	...
	//! OnPlayerAuditSuccess is the join hook (design S5: NOT OnPlayerConnected), and doing the
	...
	//! doc S2 requires ("an admin can respawn a player who died to a glitch - that path must
	//! always exist").
	//!
	...
	//------------------------------------------------------------------------------------------------
	//! Poll until possession lands (200 ms x 25 = 5 s ceiling), then announce the spawn.
	//!
