...
//! TBD events are ONE LIFE (TBD_MOD_DESIGN.md S2). Between deploy and "go" every player on the
...
class TBD_SafestartManager : SCR_BaseGameModeComponent
{
	...
	//! helper from its own setters because onRplName never fires on authority (design S5).
	...
	//! one guarded helper - the listen-host landmine from TBD_MOD_DESIGN.md S5: on a listen host
