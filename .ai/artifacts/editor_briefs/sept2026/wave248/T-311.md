You are the slice agent for ticket **T-311** (Leaderboard ORDER BY lacks tie-breaker; paging unstable) on the TBD-Reforger factory.

WORKTREE — work ONLY here, never in the main checkout:
  /run/media/system/Disk_2/Projects/TBD-Reforger/.ai/artifacts/worktrees/T-311   (branch slice/T-311)
...
FIRST THREE ACTIONS, before anything else:
  1. `cd` there, run `pwd && git branch --show-current`, paste the output in your report. If it does not show
     the worktree path and slice/T-311, STOP and report — do not start work.
  2. `export CARGO_TARGET_DIR=/home/Samuel/.cache/tbd-target` (unset in a fresh shell; a worktree building its
     ...
     instance, a CLI you exercise), build THAT into a private dir `CARGO_TARGET_DIR=/home/Samuel/.cache/tbd-target-T-311`
  ...
  3. Read the full ticket + plan + spec prompt block — they are the handoff, there is no prior chat:
       cargo xtask ticket show T-311          (or read .ai/tickets/T-311.toml)
       docs/plans/t-311_plan.md
       docs/specs/ideas/t311_leaderboard_tiebreak.md  → section "## Claude Code prompt — T-311"
     Then read CLAUDE.md §HARD GATE and docs/platform/PLATFORM_FACTORY.md sections 'THE SIGNATURE DEFECT' and 'Known traps'.

## THE TICKET
leaderboards.rs:47-51: none of the five whitelisted ORDER BY arms has a tie-breaker; with LIMIT/OFFSET over tied rows (T-194's golden makes 4-way ties) Postgres may repeat one row across pages and skip another. Append `, lt.discord_id ASC` to every arm and add a paging test over the golden.
Done means: paging the leaderboard across tied rows yields every row exactly once in a stable order; each category's SQL still comes only from the whitelist.

## YOUR FILES (touch ONLY these; new files must have exactly these names)
apps/website/api/src/handlers/telemetry/leaderboards.rs
...
T-940.5: db.rs, config.rs, .env.example · T-940.6: handlers/admin/audit.rs, services/audit_notify.rs, services/mod.rs, a migration
...
3. `cargo test -p map-engine-core` ALWAYS with `--all-features` (without it feature-gated modules never compile — vacuous pass).
   Ad-hoc tests: `cargo xtask platform wave test --slice T-311 -p <pkg>`; cross-check `--list` total vs run total.
...
5. GATE: `cargo xtask platform wave gate --slice T-311` from inside the worktree; must end `SLICE GATE: PASS`.
   "REFUSING to pass — resolved to NO crate" / "rustfmt was invoked ZERO times" means your diff has no lintable Rust,
   not that your code is broken. Also run: cargo test -p website-api leaderboards (integration tests need the dev DB on :5434; use a fresh cold DB as TEST_DATABASE_URL).
