...
Own these files under `apps/website/frontend/src/v2/apps/editor/` (except `../debug` paths, under `v2/apps/debug/`): shell/persist.rs, shell/document_commands.rs, shell/layout.rs, shell/tab_lock.rs, shell/title_prefer.rs, shell/hydrate.rs, and the five small shell doc-audit rows. Own adjacent newly extracted test files and named decomposition subfolders. Do not edit any other brief's production files.
