...
Own these files under `apps/website/frontend/src/v2/apps/editor/` (except `../debug` paths, under `v2/apps/debug/`): mission_editor.rs. Own adjacent newly extracted test files and named decomposition subfolders. Do not edit any other brief's production files.
...
- `mission_editor.rs` has many existing sibling test declarations. Move each with its owning production surface; retain those tests and their pins.
