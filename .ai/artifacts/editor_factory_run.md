...
| 203 | 126 | T-792 T-789 T-809 | PASS 30/30 (run 1; re-gate red ONCE on rustfmt of the NEW smoke → fixup 93dc7397, PASS — a 19th editor smoke, save-dialog-rect, was ADDED — **⚠️ CORRECTED 2026-08-11: the WAVE GATE RUNS NO SMOKES** (`wave.sh` has zero occurrences of 'smoke'); the editor suite runs only via `make leptos-gates`, and it is RED on expectations that predate this band. The '30/30' never included editor-suite. See T-843) | 1M/2m/1mN/1N → MAJOR fixed in-wave + focused re-verify RESOLVED; rest filed T-820..T-823 | SHIPPED — **Esc cancels zone/trigger draws** (cancel_pending's deliberate zone-survival was the mechanism; trigger shares Pending::Zone — one cancel covers both), **Save Version fresh/focused/trapped AND actually on screen** — the slice's 'did not reproduce' was WRONG-WAY (a class-read, not a measurement: the dialog centered on the 48px strip via the backdrop-filter CONTAINING BLOCK, y=-22/-184, proven by causation) — fixed with a Portal to document.body + a PERMANENT RECT SMOKE so class-string pins can never vouch for geometry again; **one asset tree per faction** with vehicle leaves + recently-placed (completion 87e1ff94: stamps record one titled entry, Add Vehicle records at its real commit site) + **the operator fold delivered** (placed vehicles in the left outliner, crew-editor stopgap held for T-818). LESSON PAIR for the record: T-791 (premise stale) vs T-789 (premise REAL) — a did-not-reproduce verdict is only as good as its MEASUREMENT. Process incident: an uncommitted orchestrator registry filing was mistaken for tree leakage by a concurrent fix agent and reverted — refiled + committed immediately; filings commit at once from now on. |
...
| 204 | 127 | T-790 T-798 T-803 | PASS 30/30 (run 1 red on rustfmt of the completion's row closure → fixup b008bfc8, PASS) | 1M/3m/1N → MAJOR FILED as T-826 (operator decision — 're-scope, not revert' per the verifier), minors T-827..T-829 | SHIPPED — **markers finally look like markers** (glyph + caption through the pinned T-760 lane, 64-alias canonical table shipped for T-806/T-808, 3-zoom signatures + centroid-exact dock-vs-map; T-790 disclosed a 6-line outside-owns edit at the mission_history feed sites — audited exact, pins intact); **validation is a top-strip chip** (headless engine, Backspace pixel-pure, load-time truth on hydrated missions, V1 copy verbatim) — but the acceptance's 'count unchanged on marker place' is UNACHIEVABLE AS WRITTEN: the first marker mints faction-{SIDE} (editor_ops:4654, outside owns) and V1 legitimately fires, measured 0→1 → the should-markers-declare-a-side DECISION is T-826, the operator's; **the drop-target layer is visible always** ('Placing into: X' with truth-telling resolvers + the completion's ROW_DROP_TARGET treatment distinct from selection on measured axes — the two-states-one-class violation closed). Completion-pass pattern used twice (row visual; recipe from the slice's instructed stop). Notable verifier by-catch: the virtual-outliner v5 smoke is config-sensitive (fails under the verifier harness, passes the gate's, both dists — T-829, pre-existing). |
...
| 205 | 128 | T-795 T-799 T-806 | PASS 30/30 (run 1) | 1M/1m/3N → filed T-832 (heading lane, folds to T-808) T-833 (ring-semantics operator decision) T-834 (residue) | SHIPPED — **Eden's widget keys** (1/2/3 six-surface-consistent — two slices + a PRE-DECLARED barrier reconciliation built to one locked map, zero disagreement in the verifier's parity sweep), **the rotate ring is a real control** (shared-const hit-test before the marquee — 54/50px boundary bites, one-undo), **exports fire once and tell one truth** (timestamp latch that passes real second clicks; per-field source table; ajv-clean held), **the marker picker collapses 64 slugs to 11 real glyphs** (compile-time count tie across the wasm boundary; pick→map handshake proven end-to-end). LESSON (third instance): T-795's heading-arrow 'not reproducing' was declared from SOURCE and was FALSE by pixels — slots render yaw-less everywhere (T-832); its translate-gizmo claim was TRUE — the difference was measurement. Ring shipped face-the-aim vs specified about-centre → T-833, operator's. |
...
| 206 | 129 | T-796 T-804 T-810 | PASS 30/30 (run 1) | 0B/0M/1N → filed T-840 | SHIPPED — the band's CLEANEST verdict. **Comments drag** (pick precedence ring→unit→comment→marquee proven at the boundaries; one txn per note; the glyph restyle rides to T-808/W207 by the band's own assignment — the amber ring persisting is design, not neglect); **the draft-saved chip** (flush-ack only on completed writes — the error branch cannot stamp; Retry-reload × draft-restore emergent pair ATTACKED AND HELD: unsaved edits survive); **the TYPE picker** (canonical writes clear ASSET-RESOLVES live; snapshot Revert via per-slot ops proven on the differing-multi case a batch would flatten; Eden axis colours; 3-press Esc ladder). Suite 1167/0 (+19). One NIT: the chip honestly reports the BOOT hydrate flush as 'saved just now' pre-edit (T-840). |
...
| 207 | 130 | T-802 T-808 T-794 | PASS 30/30 (run 1 red on rustfmt import order in an EDITION-2024 crate → fixup c6b875de, PASS) | 1M/3N → filed T-843 T-844 | SHIPPED — the VISUAL wave, three slices + **three completion passes**. **The operator's symbology language is real**: slot discs with a FACING POINT, role knockouts, top-down vehicle silhouettes per kind, neutral comment bubbles; **T-832 is dead** (9-point heading sweep monotonic and wrap-continuous, vehicles too). **Hover cursor** at ~0.03ms/move, markers honestly excluded (no selection route — corroborates T-838). **Entrance animations 215px/679px → 0.0px**, root-caused to a Tailwind v3→v4 change where `-translate-x-1/2` now COMPOSES with `transform` instead of replacing it. THREE DEFECTS FOUND BY PROVING THE WORK, NOT BY REVIEWING IT: a CLAMP-where-a-WRAP-belonged made headings 181-359 all draw due south (fixed 0bc55dc5; third instance found live on the world lanes → T-842); dragged vehicles reverted to amber discs mid-gesture (fixed f7f4a02e); marker glyphs were spilling ink across cell borders and sitting off their anchors (rebuilt from the picker's own SVGs — the picker was correct all along). **⚠️ THE WAVE'S REAL FINDING IS ABOUT THE FACTORY, NOT THE CODE (T-843).** The verifier proved `gate editor-suite` cannot run green (two stale expectations from 08-07/08-09) AND that `wave.sh` runs NO smokes at all. So the `save-dialog-rect` (w203) and `entrance-motion-rect` (w207) smokes — both perturbation-proven, both built precisely to stop a regression from going silent — are **DEAD GUARDS that nothing executes**. The orchestrator's wave-203 ledger row claimed 'the gate now runs a 19th editor smoke'; that was FALSE and is corrected above. Every `GATE PASS 30/30` in this band is accurate for the 30 steps wave.sh actually runs, and none of them ever included an editor smoke. |
| 208 | 131 | T-801 T-805 | PASS (wave.sh GATE: PASS; §5: no editor smokes in wave gate) | 0B/0M/1m/1N → filed T-846 (role_notice dead consumer) T-847 (host pin omits tether bind) | SHIPPED — **squad tethers track the drag preview** (affected endpoints only; multi-select both ends; Class-R packer pins perturbation-proven; live 4px CDP/rf left to eye-pass). **Editor route enforces mission_maker** (enlisted/leader/guest redirect + toast; maker+admin pass; draft-8 REFUTED as sync seed stay-local, not IDB leak). Last wave of the original UX band. Coders+verifier: Grok 4.5 (operator override of Opus/Fable default). **Eye-pass 2026-08-11:** T-805 PASS. T-801 FAIL (tether did not follow — T-850). Connect Group to allows multi-squad-looking links + Sync suspect (T-848); no ungroup (T-849). Debug stutter acknowledged by operator (own fix pending — not filed).
| 210 | 132 | T-818 T-819 T-836 | PASS (run 1 red on 71 Class-R pins — T-819 scrub-anchor literal duplicated MissionEditorPage; completion ddbdcddd+00eea875, then PASS; §5: no editor smokes) | 0B/0M/2N → filed T-851 T-852 | SHIPPED — **vehicle crew/heading/cargo live in Attributes**; right-dock Placed strip dead; **crewed figures leave the map** (derived SoA hide; compile/outliner/selection intact); **seed veh: aliases** so T-800 vehicles compile. Coders+verifier: Grok 4.5.
...
**Queued and ready:** waves **211-217** — 26 tickets, rows in `wave_plan.tsv`, every wave
