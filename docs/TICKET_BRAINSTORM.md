...
- **T-925** (idea) — Close-selection semantics: the ceremony is unreachable live [] — current_wave names the first wave holding an unshipped ticket, so it always refuses to close, and a fully-shipped batch's label dissolves into wave 0 at the ship-hook repack. Decide what wave close selects.
