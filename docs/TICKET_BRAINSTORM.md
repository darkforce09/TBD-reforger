...
- **T-405** (deferred) — T-391 closed the write boundary but not the live XSS — the frontend sink and stored rows are unremediated [API, FRONTEND, DATA] — MAJOR, found by wave 2's adversarial verifier against merged main. THE TOP ITEM OF THIS BATCH. Do not read "T-391 shipped" as "the XSS is closed".
