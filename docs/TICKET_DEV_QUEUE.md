...
## T-370 — Remove the 8 dead mark_adopted call sites — but move the purge first

- **Slice spec:** ``
- **Program hub:** ``
- **Branch:** `ticket/T-370`
- **Targets:** website
- **Summary:** FOLLOW-UP from T-352, which deliberately left this and explained why. **The sequencing is the whole ticket; getting it wrong strands data in users' browsers permanently.**
...
## T-573 — Mixed drag: vehicle GPU preview still slots-only (set_drag SoA)

- **Slice spec:** ``
- **Program hub:** ``
- **Branch:** `ticket/T-573`
- **Targets:** website
- **Summary:** FOUND by W68 adversarial verifier (CLEAN NIT) after T-491, and admitted by the slice agent.
...
## T-625 — Behind Caddy every public client shares one auth rate-limit bucket

- **Slice spec:** ``
- **Program hub:** ``
- **Branch:** `ticket/T-625`
- **Targets:** website
- **Summary:** FOUND by wave 81''s verifier (F5). **Pre-existing and self-documented**, but worth knowing before a
big event night.
...
## T-626 — T-587 backfill regex is not character-for-character with parse_grid

- **Slice spec:** ``
- **Program hub:** ``
- **Branch:** `ticket/T-626`
- **Targets:** website
- **Summary:** FOUND by wave 81''s verifier (F3). Divergence is entirely in the **safe** direction; filed for accuracy.
