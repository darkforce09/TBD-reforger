...
**Shipped:** T-122 fixes @ `f131770` (branch `ticket/T-122`). `cargo xtask db test-it` ok; frontend `npm run build` + `npm run lint` clean. Go `go build ./...` clean.
...
**Mod note:** Enfusion `.c` changes (C4, T14/T16/T17, M8–M15) are not covered by `cargo xtask db test-it`/frontend build and need a Workbench pass to validate at runtime.
