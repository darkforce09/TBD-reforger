# Monorepo Migration Runbook

Migration program **M0–M3** merges `TBD_Website` + `Arma reforger` into this repo without rewriting individual commit SHAs from the source repos (read-tree merge).

## Gates

| Gate | When | Checks |
|------|------|--------|
| **G0.5** | Before M1 | Both originals committed; SHAs in [`artifacts/migration-baseline/G0-SHAs.md`](../../.ai/artifacts/migration-baseline/G0-SHAs.md); push to GitHub |
| **G1** | After read-tree merge | V1–V3 content parity |
| **G2** | After M1b lift | V4–V5, V17, V25 |
| **G3** | After M1c path fixes | V11–V22; executor gate in CLAUDE + AI_PLAYBOOK |
| **G4** | After M2 ticket rewrite | V6–V10, V23–V27; `./scripts/ticket check --strict` |
| **G5** | Before workspace switch | Full `./scripts/verify-monorepo-migration.sh` exit 0 |

G5 used `make verify-migration` / `./scripts/verify-monorepo-migration.sh` (both deleted with the Makefile / T-853 ports). The migration is complete. Current commands: root [`CLAUDE.md`](../../CLAUDE.md) §Run it locally (`cargo xtask help`).

## Layout after migration

> **Superseded by the `2a51d66` workspace restructure (2026-06-26):** the tree below was the
> M0–M3 end state. The repo has since been reorganized into `apps/` + `packages/` + unified
> `docs/` + `scripts/` + hidden `.ai/`. The current layout is:

```
TBD-Reforger/
├── apps/
│   ├── website/      # Rust API + Leptos SPA (was TBD_Website)
│   └── mod/          # tbd-framework + deploy scripts (was Arma reforger)
├── packages/tbd-schema/
├── docs/             # specs/ + platform/ + backend/ + website/ + mod/
├── scripts/          # mod/ + website/ + deploy/ + ticket
├── .ai/              # tickets/ (unified registry) + artifacts/
└── CLAUDE.md         # canonical agent context
```

## Path rewrites

| Old | New |
|-----|-----|
| Legacy path (pre-monorepo) | Monorepo path |
|----------------------------|---------------|
| Design_Docs tree | `docs/specs/` |
| `mod/tbd-schema/` | `shared/tbd-schema/` |
| embedded web tree | `website/` |
| `../worktrees` | `artifacts/worktrees` |

Scripts (one-shot path rewrites used during the migration): **removed in T-162** —
do not resurrect `rewrite-ticket-paths.py` / `rewrite-doc-links.py` /
`backfill-registry-monorepo.py`. Use `./scripts/ticket` / `cargo xtask` for registry work.

## Local-only paths (not in git)

These exist on disk for local dev but are **never tracked**. A filesystem `diff -qr` against the old repos will show them as "missing" from git — that is expected.

| Path (current `apps/` layout) | How to get it |
|-------------------------------|----------------|
| **`apps/mod/crf_framework/`** | Copied during M1 from the original mod repo. Workbench reference only. |
| ~~`apps/mod/tbd-framework/Scripts/WorkbenchGame/EnfusionMCP/`~~ | **No longer local-only (2026-09-12).** The ~19 `.c` handlers are committed at `apps/mod/tbd-emcp/Scripts/WorkbenchGame/EnfusionMCP/`; nothing to run after clone. |
| **`tools_v2/xtask/deploy/deploy.env`** | Copy from `deploy.env.example` beside it (secrets). |
| **`apps/website/.env`** | Copy from `.env.example`. |
| **`apps/mod/.local-test-profile/`** | Generated by `cargo xtask setup server-profile`. |

Git-tracked parity vs originals: website app files match (minus lifted trees); mod app files match (minus lifted schema CI). Empty Mission Creator placeholder dirs (`arsenal/`, `registry/`, etc.) were never in git.

## Post-G5 (operator)

1. Create GitHub repo `darkforce09/TBD-Reforger`
2. Push monorepo `main`
3. Archive old remotes (read-only)
4. Point Cursor workspace at `/run/media/system/Disk_2/Projects/TBD-Reforger/`
5. Resume normal split: Cursor docs, Claude Code `executor: claude-code` slices
6. Update staging server clone path on `192.168.0.140` after cutover (`deploy-staging.sh` rsyncs from monorepo root)
