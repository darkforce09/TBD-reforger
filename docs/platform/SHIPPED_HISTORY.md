...
reads of files this size were the single largest token sink in the program.

**Accuracy warning, preserved deliberately.** Entries below narrate stacks that no longer exist:
the Go backend (removed at T-145) and the React/TypeScript frontend (deleted at T-159.29.3). The
repo now tracks zero `.go`, `.ts`, `.tsx` and `.js` files. Read every entry as a record of what was
true when it was written, not as a description of the codebase today. Each entry carries its commit
sha, so `git show <sha>` is the authority.

---

## Status — shipped slices (archive)
