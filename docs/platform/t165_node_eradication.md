...
| T-165.4 | **IN PROGRESS** — **.4a shipped**: core `encode_tbdd` promoted (pub, 4 codec tests); tbd-tools compute libs `geometry` (anchor-check port), `density` (constants + corner grid + fixture round-trip), `forest` (275-LOC derivation + ring trace, 1:1). Remaining: S2–S14 gate, glyphs, terrain-alignment/DEM, label gates+libs; then `make schema-validate` pure cargo | next |
