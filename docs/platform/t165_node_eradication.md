...
| T-165.4 | **IN PROGRESS** — .4a libs · .4b S-gate (12/12 parity) · **.4c**: glyphs gate (parity + probe) + **height-labels gate with the wasm-era branch RESTORED natively** (G5/G6 declutter + ASL oracle + G3 completeness via map-engine-core — the Node runner permanently skips these; probe proved Rust strictly stronger) — **`make schema-validate` = 100% cargo, zero node calls**. Remaining .4 tail: town/road/locations label gates, terrain-alignment + dem-sample + raw-u16-to-dem-png | next |
