...
**Partial coverage:** **T-124** shipped deps/toolchain; `cargo xtask db up` / `cargo xtask mk rust-api` / `cargo xtask mk leptos` documented in CLAUDE.md; every mod command is a `cargo xtask mod` subcommand.
