...
| **Coding standards (T-125)** | [`docs/platform/CODING_STANDARDS.md`](../platform/CODING_STANDARDS.md) + [`t125_coding_standards_enforcement.md`](../platform/t125_coding_standards_enforcement.md) — before commit: `make ci-local` green (needs `make db-up` + `nvm use` + `editorconfig-checker` in PATH); includes `verify-editorconfig`, `verify-coding-standards`, `npm run format:check` |
...
- `apps/website/frontend/src/stitch-exports/**` (archive)
...
- **Do not create** markdown spec trees under `apps/**/docs/` or `packages/**/docs/` — surface specs live in [`docs/website/frontend/`](frontend/) per [`DOCUMENTATION_STANDARDS.md`](../platform/DOCUMENTATION_STANDARDS.md) §8.2

Live UI authority: `apps/website/frontend/src/pages/` + `apps/website/frontend/src/features/`.
