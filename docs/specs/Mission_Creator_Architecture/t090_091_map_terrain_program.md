...
**Status:** **active — T-090.3.0** (Workbench export spike). **T-090.1** (aligned basemap) **queued** until 3.0 K1–K7 PASS. **T-091 complete** @ `dde589e`. **T-090 expanded** (2026-06-26): multi-slice **map visualization program** — tiles + typed world objects + Eden-like render.  
...
T-090.0.2  map-object schemas + goldens + verify wiring (shipped @ this pass)  ✓
  ...
  → T-090.3.0  Workbench world-export feasibility spike  ← ACTIVE NOW
  → T-090.1    Satellite basemap [queued until 0.2 + 3.0 PASS]
  → T-090.1.1  Map basemap + Satellite/Map switch (synthesized fallback per N9)
...
**Blocker chain (implementers):** **T-090.0.2** (schema+golden, shipped) → **T-090.3.0** (spike, active)
→ **T-090.1** (basemap, queued until 0.2 + 3.0) → **T-090.1.1** (Map, after 0.2) → **T-090.2** →
**T-090.3** → {**T-090.4**, **T-090.6**, **T-090.8**} → **T-090.5** → **T-090.9** → **T-090.7**.
...
| **T-090.1** | [`t090_1_aligned_basemap.md`](t090_1_aligned_basemap.md) | claude-code | **Satellite** pyramid — **queued** (after T-090.3.0) |
...
| **T-090.3.0** | [`t090_3_0_workbench_export_spike.md`](t090_3_0_workbench_export_spike.md) | claude-code | **spike** — enumerate + OBB + tiles + forest mask (K1–K7) |
...
make map-census TERRAIN=everon   # exact object counts (pending until T-090.3.0 export)
