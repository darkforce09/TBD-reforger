...
| **After export** | `cargo run -q -p tbd-tools --bin world -- census --terrain <id>` writes **exact** non-negative integers. Re-census without re-export → **byte-identical** JSON (I6). |
| **Phase ship** | `cargo run -q -p tbd-tools --bin world -- verify-phase` compares export row counts to inventory — **integer equality** (G11). |
...
2. cargo run -q -p tbd-tools --bin world -- census --terrain everon
...
cargo run -q -p tbd-tools --bin world -- census --terrain everon --phase P1_buildings   # scoped to phase filter
```
...
Runs as part of `cargo run -q -p tbd-tools --bin world -- verify-phase` and `cargo xtask ci schema-validate` (`verify-type-inventory.mjs`).
...
**Fill this table from `type-inventory.json` after T-090.3.0 spike + first `cargo run -q -p tbd-tools --bin world -- census --terrain everon`.** Commit the updated JSON; run `./scripts/ticket sync` if registry ops log references counts.
...
| 3 | `cargo run -q -p tbd-tools --bin world -- census` Makefile target |
