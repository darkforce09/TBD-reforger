...
| T-825 | 5070 | deferred | work | Design session scopes universal outliner list Attributes edit rule | PROGRAM: everything placed is an outliner citizen; all editing is Attributes; the right dock only places |
...
| T-923 | 6030 | shipped | work | wave close writes the marker commit itself | platform wave close writes the marker commit itself: subject self-checked against wave_close_subject_ok and oracle 1 before committing, then wave repack and an auto-committed lock refresh. Dirty tree refuses; --dry-run prints the subject and stops. |
| T-925 | 6040 | shipped | work | Close-selection semantics: the ceremony is unreachable live | wave close targets the oldest pending emptied label: the ship-hook repack records a fully-shipped wave's label and frozen ticket set in the lock, close validates and closes exactly that set, and open waves number past pending labels. |
| T-934 | 6100 | shipped | program | Website reorganization — `docs/specs/website_reorg/website_architecture_audit.md` §3/§4 | Restructure flat 73-file frontend into core/shell/pages/editor, decompose mission_editor/editor_ops/arsenal monoliths, extract gestures via EditorGestureContext, nest backend handlers by domain. |
| T-070 | 6439 | cancelled | work | Vehicles placeable | Drag vehicles from palette onto map with crew hooks. |
| T-080 | 6440 | cancelled | work | Connection / sync UI | Entity connection lines and sync group authoring. |
| T-081 | 6441 | cancelled | work | Transform widget + snap grid | On-map transform gizmo and snap grid. |
| T-137 | 6442 | deferred | work | Discord platform rework | Discord structure + bot flows: slot confirm, reminders, AAR links. Complements T-118 website slotting; mostly ops/human. |
| T-238 | 6443 | cancelled | work | Add ticket check to CI | make ticket-check exists but grep of .github/workflows for ticket returns nothing, so nothing prevents the registry drifting again. |
| T-940 | 6999 | queued | program | Website platform: events, telemetry, admin, content | Fixes the audit's S5 findings in apps/website/api: waitlist seats, registration tombstones and no_show, schedule cascades, flat telemetry counters, pool config, audit stream, users pager, vehicle mutations, wiki features, mortar ballistics, rcon actions, reservation tiers and telemetry events. |
| T-311 | 7020 | shipped | work | Leaderboard ORDER BY lacks tie-breaker; paging unstable | None of the five ORDER BY arms in handlers/telemetry/leaderboards.rs:47-51 has a tie-breaker, so LIMIT/OFFSET paging returns an unstable row set once T-194's golden introduces 4-way ties. Append `, lt.discord_id ASC` to each arm and test paging. |
| T-305 | 7030 | shipped | work | pak.rs seeks data_start plus offset; offsets are absolute | read_file/read_raw in tools/tbd-tools/src/world/pak.rs seek data_start + entry.offset, but entry offsets are absolute from the pak start; every `enf extract` read is shifted by 56 bytes. Seek entry.offset directly, pinned by a fixture test. |
| T-298 | 7040 | shipped | work | Gate tbd-tools density tests in CI | `cargo test -p tbd-tools --lib density::` passes on main today (2 tests, 2026-09-05) but no CI lane runs it; add the tbd-tools lib tests to ci.yml and pin the corner-partition invariant with a seeded randomized test. |
| T-943 | 7050 | shipped | work | platform wave push deadlocks on large LFS ranges | cmd_push's LFS guard pipes every path of a commit into git check-attr without draining stdout; a commit touching 1,691 files wedges both processes forever, and on a host with git-lfs the guard refuses a legitimate push. |
| T-300 | 7060 | shipped | work | Shared CARGO_TARGET_DIR serves unmerged slice binaries | A binary built inside a slice worktree counts as fresh for a main-checkout build under the shared CARGO_TARGET_DIR, so run-style lanes can execute unmerged code; run-only binaries get their own target and preflight checks the build stamp. |
| T-924 | 7070 | shipped | work | Gate verdict receipt required at land | platform wave gate writes a verdict file (HEAD sha plus verdict) and platform wave land refuses a landing ticket without a fresh one. Forward-only: future lands only, no backfill, existing tickets stay green untouched. |
| T-090 | 7079 | ready | program | Map visualization program | Map Engine v2 through sea-band + contours @ `bd481cf1`. **Active:** **T-090.5.5** tree/veg/prop glyphs. Single lane. |
...
| T-681 | 7340 | shipped | work | Entity states: health, allow-damage, show-model, size, stamina | Entity states: health, allow-damage, show-model, size, stamina |
...
| T-685 | 7370 | shipped | work | Zone volumes: height bounds capture counts and owner | Zones become volumes: min/max height, attacker and defender counts, starting owner |
| T-689 | 7380 | shipped | work | Play-area enforcement gains a vehicle-class axis: the aircraft exemption | Play-area enforcement gains a vehicle-class axis: the aircraft exemption |
...
| T-675 | 7440 | queued | program | Vehicle roster reaches game — T-076 compile half | Cross-boundary compile half split from T-076 so crew UI could ship separately.
...
Unmerged salvage 113108a1 overlaps T-674 — check subsumption before redo.
 |
| T-936 | 7449 | queued | program | Mission logic the audit found missing | Win/loss framework, task state machine with HUD, editable radio nets, weather timeline, audio emitters, dynamic spawn modules and tactical graphics — schema, core flatten, editor panels and Enfusion runtime for each. |
| T-299 | 7480 | shipped | work | Single-faction compile ships a phantom opfor | flatten pads a stub opposing faction (flatten.rs:2470-2472) to satisfy the schema's two-faction rule, so every single-faction mission shows an unplayable side in briefing and ORBAT. Let the schema permit one faction and stop padding. |
| T-310 | 7490 | shipped | work | Arsenal attachments never reach the compiled document | T-197 persists weapon attachment edges in SlotLoadoutV2 but flatten's mod_slot_loadout reads only weapon/optic/magazine and the schema gear block has no list; add gear.attachments[], emit it, and mount it in TBD_LoadoutEquipHelper.c. |
| T-290 | 7540 | ready | work | Nine dead flatten fields mod never reads | meta.author/templateId/playerRange, the environment block, factions[].tickets, orbat[].type, winConditions.mode, flow.briefingSeconds and the orbat block are emitted but never read by the mod; add readers where cheap and annotate the rest in an emit ledger. |
| T-291 | 7550 | shipped | work | Resolve five schema fields implemented on no surface | environment.windDirDeg, factions[].color, roles[].radio, layers[], and settings.{respawn,spectatorPolicy,nightVision} are declared in the contract with zero implementation in flatten or the mod. spectatorPolicy is dead despite Spectator/ being a shipped seven-file subsystem. |
| T-242 | 7560 | shipped | work | Emit T-216 slot deltas through flatten | The T-216 schema deltas (slot tag/callsign/rank/stance, group leaderSlotId, root vehicles $def) are all declared in mission.schema.json today, but flatten drops the four slot fields (DIAG_DROP_SLOT_*); emit them and pin with schema-validated goldens. |
| T-133 | 7570 | shipped | work | OFCR timed objectives | Objectives that evaluate at mission time T+N: the T-936.2 task model gains a schedule (delay, window), the tasks panel edits it, and TBD_TaskStateMachine.c fires the transition on the mission clock. |
| T-941 | 7579 | queued | program | Enfusion mod lifecycle: safestart, lobby, screens, HUD | Fixes the audit's S6 findings in apps/mod/tbd-framework: safestart armed through LOBBY and BRIEFING, once-only deploy on BRIEFING, END and DEBRIEF screens, an objective HUD, spectator clamp, private link codes, radio fallback and fuelled vehicles. |
| T-301 | 7660 | shipped | work | Briefing kit lists 7 of 13 gear fields | TBD_BriefingData.BuildKit (UI/TBD_BriefingData.c:519) lists seven gear fields; T-182's launcher, handgun and throwable never appear on the briefing screen. Add them while keeping pants/boots/handwear hidden. |
| T-302 | 7670 | shipped | work | Prove T-182 weapon equip on a live body | T-182's slot-indexed weapon equip compiles but was never observed in game; add a per-slot equip result log, a headless world-boot assertion that four authored weapons land in four slots, and a human checklist item. |
| T-304 | 7680 | shipped | work | RegistryScan never reads weapon weight; 32 wrong rows | TBD_RegistryScan ReadPhysAttrsPass treats SCR_WeaponAttachmentsStorageComponent as storage so weapon weight is never read (0/107), and its class-keyed foreach picks hash order over derivation depth, so 32 rows carry Item_Base.et's 0.01 kg. |
| T-139 | 7690 | shipped | work | Lobby loadout visual preview | In-game lobby kit preview: a TBD_LoadoutPreview widget renders the selected slot's gear as an icon grid (stretch: 2D mannequin) beside the slot list in TBD_LobbyScreen; layout file under UI/layouts. |
| T-937 | 7699 | queued | program | Editor data layer: id arrays, undo, persist | Fixes the audit's S2 findings in the mission editor data layer: native YArray id lists, gesture-grouped undo with a depth cap, a materialize side-key cache, surfaced save errors with hidden-tab flush, and payload schema hardening. |
| T-257 | 7750 | shipped | work | Undo scope misses loadouts, items, objectives, markers | hydrate clears loadouts, items, objectives and markers but store.rs expand_scope covers slots, squads, factions and editor_layers only, so those roots become non-undoable the day something mutates them. Add them to the UndoManager scope with a test per root. |
| T-190 | 7760 | shipped | work | Two tabs on one mission clobber each other | Two tabs on one mission write one IndexedDB key; the last debounce wins and the reload prompt blames server drift. Detect the second tab, merge through the CRDT, and give the conflict modal counts, timestamps and a destructive marker. |
| T-930 | 7790 | shipped | work | Vehicle first-paint disc until moved | Vehicle first-paint disc until moved — W210 eye-pass |
| T-938 | 7799 | queued | program | Engine and wasm performance | Closes the audit's S3 render and geometry findings: pooled lane buffers, measured chunk-crossing uploads, GPU culling for every icon lane, BVH section cuts with a sparse HeightField, frame-sliced viewsheds, and a wasm memory budget guard. |
| T-939 | 7859 | queued | program | Editor usability: selection, gizmo, arrange, templates | Fixes the audit's S4 findings in the mission editor UI: outliner multi-select drag, batch faction and squad reassign, a Z gizmo, Arrange shortcuts, squad templates, canvas diagnostics with connection wires, vehicle-panel virtualization and Ctrl+F search. |
| T-932 | 7875 | queued | work | Parked briefing markers survive server save/reload | T-826 parking: pendingBriefingMarkers drop on server JSON save/reload before first faction mint |
| T-704 | 7940 | ready | work | Command palette over every editor command | Alt+Space opens a fuzzy palette over every top-strip and toolbelt command, ranked by usage frequency stored per browser; Esc closes; Enter runs the command through its existing handler. |
| T-142 | 7950 | ready | work | MC shell layout polish | Toolbelt placement, Attributes modal grouping and stub-tool visibility — layout UX without full T-082 field parity; no document-write changes. |
| T-157 | 7960 | ready | work | Mission Create visual overhaul | The New Mission dialog gets a visual map picker with terrain thumbnails and a modset selection; time, weather and max players leave the dialog because they are editor settings. |
| T-158 | 7970 | ready | work | Editor shell UX consolidation | Consolidate the two Settings entry points into one, wire the top-bar buttons that are still inert, and remove the redundant left-dock Assets tab now that the right dock is the asset surface. |
| T-129 | 7980 | ready | work | Building floor selector | Per-building floor slice in the Mission Creator map: a floor selector tool derives storey bands from the occluder BVH heights and clips the building render to the chosen floor so placement inside multi-floor buildings is unambiguous. |
| T-131 | 7990 | ready | work | Route planner tool | Mission Creator tool that plans routes on the exported road graph: click waypoints, snap to road segments, show distance and an elevation profile. Not runtime convoy AI. |
| T-141 | 8000 | ready | work | Procedural slot naming | Adjective/nickname word packs generate slot display names on creation; a manual override wins and survives renumbering. Builds on T-071 ORBAT numbering. |
| T-146 | 8050 | ready | work | Asset Browser Data Wiring | Hook up T-150 registry items (vehicles/crates/…) to Asset Browser for map drag-place. Unblocked by T-150; after or parallel T-068.9 ingest preferred. |
| T-143 | 8070 | ready | work | Water mask placement guard and exact hydrology | Ship the T-935.9 water mask to the editor: a placement guard refuses units in the ocean and warns in lakes; pak.rs (fixed by T-305) reads Eden water entities so the vector layer becomes exact instead of the appearance classifier. |
| T-135 | 8180 | ready | work | Mission modset manager | Per-mission Workshop modset presets validated against registry aliases at export: a workshop_sync service resolves mod metadata, the modpacks page edits presets, and export refuses a mission whose registry aliases are not covered by its preset. |
| T-309 | 8190 | ready | work | FactionDoc squad level for Apply Template | Preserve squads through Apply Template: add a squad level to faction-library.schema.json and FactionDoc (core/dto.rs:745), then both sides of the editor ops — orbat_apply_faction and faction_doc_from_side_core — so save-as-template then Apply round-trips N squads. |
| T-140 | 8200 | ready | work | Mission client payload budget | Measure compiled mission size against slot and entity counts, define a console-safe budget, and make compile report a payload-budget diagnostic with a server-only-bulk policy for what exceeds it. |
| T-136 | 8210 | ready | work | 3D AAR / OCAP-style replay | Post-event replay: T-940.13 telemetry events → a replay endpoint returning a per-match timeline → a map scrubber page with unit positions over time; aar_replay_url on deployments links to it. |
| T-295 | 8220 | ready | work | Realtime collaborative editing | Axum websocket at /missions/{id}/sync relays yrs updates and awareness between clients; the editor's collab_sync.rs applies remote updates into the local doc; POST /versions gains edit-since-load concurrency (base version id → 409). |
| T-132 | 8230 | ready | work | Multiplayer MC + visual git | Visual mission diff/review UI over the T-295 sync: doc/diff.rs computes entity-level changes between two versions and a mission_diff page renders added, removed, moved and changed slots and vehicles for review before publish. |
| T-138 | 8240 | ready | work | One-command self-host setup via xtask | `cargo xtask setup all` checks deps, writes .env, runs db up, migrate and seed, and optionally pulls map assets (git lfs pull) — the fork-friendly install story, in Rust, no shell script. |
| T-294 | 8250 | ready | work | Arland has a manifest and no object data | terrain-registry.json declares arland queued with P1 only; packages/map-assets/arland holds a 756-byte manifest and zero objects. Run the export-terrain gate for arland: the operator exports from Workbench, the agent stages, builds and records P1. |
| T-653 | 8260 | ready | work | Preserve the three headless editor-screenshot findings | Document in EDITOR_GATE_RUNBOOK.md and KB-002 the three non-obvious headless screenshot fixes: writable XDG_CACHE_HOME for fontconfig, --use-angle=vulkan only, and canvas.toDataURL capture (Page.captureScreenshot returns a black map). |
| T-111 | 8269 | cancelled | program | Lazy chunk residency @ 1M | Program for T-067.1: byte-budget chunk eviction with cache eviction and reload on viewport re-entry, so a 1M-object terrain stays within browser memory. Spec: t067_spatial_chunks.md §Deferred. |
| T-905 | 8280 | ready | work | Split 37 T-159 frontend SIZE-3 allowlist files | Split T-159 frontend SIZE-3 files (37) the T-899 allowlist is holding |
| T-906 | 8290 | ready | work | Split 7 T-145 API SIZE-3 allowlist files | Split T-145 API SIZE-3 files (7) the T-899 allowlist is holding |
| T-907 | 8300 | ready | work | Split 11 map-engine SIZE-3 allowlist files | Split map-engine SIZE-3 files (11) the T-899 allowlist is holding |
| T-908 | 8310 | ready | work | Split 8 xtask/tools SIZE-3 allowlist files | Split xtask/tools SIZE-3 files (8) the T-899 allowlist is holding |
| T-933 | 8320 | ready | work | leptos-gates v-suite goldens break pre-close exit-0 | mk leptos-gates exits non-zero after editor-suite 20/20 because v-suite SPA goldens mass-fail |
| T-816 | 8330 | ready | work | Armed composition hint open; one Esc clears both layers wrongly | Esc with an armed composition and the Controls Hint open clears both in one press |
| T-817 | 8340 | ready | work | Grid labels lag ~1.4s on stationary wheel zoom | Grid labels lag up to ~1.4s on wheel zoom with a stationary pointer |
| T-820 | 8350 | ready | work | Catalog failure generic cause; chips visible wrongly | Catalog failure view shows the generic cause with chips visible where no-modpack should fire |
| T-821 | 8360 | ready | work | Save version prefill static; second save 409s | Save Version prefill is a static 0.1.0 — no auto-bump, second save 409s |
| T-822 | 8370 | ready | work | Outliner dblclick must not open asset picker under Attributes | Outliner dblclick bubbles into the map container — asset picker opens under Attributes |
| T-823 | 8380 | ready | work | OBJ readout must count vehicles or rename honestly | OBJ readout counts slots only — placed vehicles are invisible to it |
| T-824 | 8390 | ready | work | Placed zones must render visibly at rest on map | Placed zones are not visible on the map — investigate and fix the render (repro-first) |
| T-827 | 8400 | ready | work | Validation chip red under 4.5:1 live-effective | Validation chip red measures 4.0-4.3:1 over the live glass — passes only by plate-calc |
| T-828 | 8410 | ready | work | Marker captions drift when zoom changes without rebind | Marker captions are world-frozen at bind-time zoom — drift past 40px until a rebind |
| T-830 | 8420 | ready | work | Outliner rows cramped; density pass on layer tree | Outliner rows are too cramped — density pass on the layer tree |
| T-831 | 8430 | ready | work | Per-side marker authoring audit then explicit UI | Per-side markers: author for each side, visible only to that side (Arma 3 model) — establish what exists first |
| T-833 | 8440 | ready | work | Rotation ring: relative delta plus live preview | Rotation widget: relative drag delta (not absolute aim) + live per-frame preview |
| T-834 | 8450 | ready | work | Wave-205 residue: two stale pre-renumber comments, dead widget_is_rotate, marker grammar | Wave-205 residue: two stale pre-renumber comments, dead widget_is_rotate, marker grammar |
| T-837 | 8460 | ready | work | Vehicles cannot be deleted — slots can, vehicles cannot | Vehicles cannot be deleted — slots can, vehicles cannot |
| T-838 | 8470 | ready | work | Map markers selectable; outliner lists; dblclick opens Attributes | Markers select on the map and list in the outliner; double-click opens marker Attributes |
| T-839 | 8480 | ready | work | Retire floating Select/Ruler/LoS bottom-centre pill | The floating Select/Ruler/LoS pill still exists — decision 1 said it dies |
| T-841 | 8490 | ready | work | Type picker popover translucent; make opaque panel | The Type picker popover is translucent — text blurs over the map; make it an opaque panel |
| T-845 | 8500 | ready | work | A selected vehicle looks identical to an unselected one | A selected vehicle looks identical to an unselected one |
| T-848 | 8510 | ready | work | Group to must use exclusive ORBAT membership | Connect ▸ Group to must mean exclusive ORBAT squad membership — not stackable connection edges |
| T-849 | 8520 | ready | work | Add ungroup leave-squad verb without deleting slot | Ungroup / leave-squad — no editor verb to take a slot out of a squad without deleting it |
| T-850 | 8530 | ready | work | Squad tether must follow drag on auto-grouped units | T-801 eye-pass: squad tether did not follow drag on same-squad (auto-grouped) units |
| T-926 | 8540 | ready | work | Vehicle Attributes Transform/Position tab | Vehicle Attributes need Transform/Position parity (X/Y/Z + Heading at minimum); T-818 only shipped Heading/Cargo/Crew |
...
| T-981 | 9810 | cancelled | work | Occluder archive cannot carry blocking prefabs | CANCELLED — REFUTED 2026-09-06. Blocking prefabs keep the JSON descriptor lane; archive was never meant to carry them. |
| T-946 | — | idea | program | Wave close: child tickets, ledger-seated labels, batch ship | wave --close could not close any wave: the close-time view saw parent tickets only, lock labels ran past what the marker oracle accepts, and per-id repacks stopped a wave ever emptying. |
| T-947 | — | idea | work | World-parity DEM skip passes green and silently | xtask/src/map_world_los_tests.rs:200 returns green with zero assertions when load_dem errs. The gate vacuity guard only matches a leading skip: prefix and only wraps test api, so nothing catches it. |
| T-948 | — | idea | work | audit_notify tests pass green with no database | apps/website/api/tests/audit_notify.rs skips six DB tests when TEST_DATABASE_URL is unset and reports ok in 0.01s. T-311 wrote the opposite rule in the same wave: a missing URL is a FAIL. |
| T-949 | — | idea | work | Leaderboard paging test cannot fail on its named claim | tests/leaderboards_paging.rs asserts page-set completeness on a six-row board, where both orderings return the same set. Only the in_order assertion has red power. Seed tied rows or rename it. |
| T-950 | — | idea | work | Audit SSE stream has no client | T-940.6 ships /audit-logs/stream but no frontend consumes it; the admin audit page is a paged before= Resource. Either wire the page to the stream or record the endpoint as API-only. |
| T-951 | — | idea | work | Audit listener registry is never pruned | services/audit_notify.rs keys a listener map on the Arc address of connect_options and never removes entries. One leaks per pool created, and a freed address can be recycled into a new pool key. |
| T-952 | — | idea | work | website-api set_var mutates a shared test process | apps/website/api/src/db.rs uses set_var under a mutex in a 281-test binary. The SAFETY note claims no C getenv reader, which is unverifiable when libc is linked. Use the existing from_lookup injection instead. |
| T-953 | — | idea | work | Migration numbering gap 0022-0024 | apps/website/api/migrations jumps 0021 to 0025. A sibling slice landing 0022-0024 later would apply after 0025 on any migrated database, and two slices can collide on one number. |
| T-954 | — | idea | work | lfs_paths_in_range never checks check-attr answered for every path | xtask/src/wave/push.rs walks check-attr triples without comparing their count to the paths fed. A truncated answer from a zero-exit child reads as nothing-is-LFS. Count triples against the NUL-terminated input. |
| T-955 | — | idea | work | git-lfs probe misses the operator's Homebrew install | xtask/src/wave/push.rs probes git lfs version on PATH, which fails under the factory's pinned PATH even though git-lfs 3.7.1 exists. The normal-push branch never runs. Probe the pre-push hook or an absolute path. |
| T-956 | — | idea | work | ci-local no longer mirrors ci.yml | mk_ci_tasks.rs and CLAUDE.md both say ci-local mirrors ci.yml, but T-298 added the tbd-tools lane to the workflow only, and no gate asserts parity. Add the task to ci-local or drop the mirror claim. |
| T-957 | — | idea | work | Vanilla reference is rotated pre-T-305 output | 2483 extracted .c files under apps/mod/vanilla_reference/Scripts are shifted 56 bytes by the pre-T-305 pak reader, and the committed enf-index TSVs were built over them. Re-extract, re-index, recommit the TSVs. |
| T-958 | — | idea | work | Density lane count comment says 57, list says 61 | mk_ci_tasks.rs describes the tbd-tools lane as 57 tests; cargo test --lib --list reports 61 after T-305 added four. Correct the comment. |
| T-959 | — | idea | work | Reroute run lanes onto the run target | Four lanes still build into the shared cache: ci_editor_api.rs api build and run, gate_mcp_call_selftest.rs mcpd, three hostrun verify steps in wave/gate.rs, and mk_build.rs rust-api. Each needs TBD_RUN_TARGET_DIR, one line per site. |
| T-960 | — | idea | work | Road census is unreachable from the classify rules | Roads export as prefab-less RoadEntity rows, so classify.rs never sees them, and build.rs hardcodes segments 0 and byRoadClass empty. Wire both from the 887 segments build_roads_from_topo already computes. |
| T-961 | — | idea | work | utility kind is dropped by the P5 phase filter | phase_kinds P5_props omits utility, so build.rs deletes utility rows instead of counting them. Adding it yields 107 prefabs and 7145 instances, and needs the store.rs prefab-count pin re-pinned. |
| T-962 | — | idea | work | Road segment count is 887, not 888 | build.rs comment and four smokes.rs assertions claim 888 segments; roads.json.gz ships 887. The live smoke asserts road_segments === 888. |
| T-963 | — | idea | work | Run stamp covers a directory, not each binary | One tbd-built-from describes a whole profile dir while cargo build compiles only what argv names, so a mixed-provenance dir reads green. The stamp also ignores a dirty tree. Stamp per binary and record dirty. Blocks T-959. |
| T-964 | — | idea | work | exact_tree_count_chunk has no zoom gate | density_ladder.rs gates vegetation by zoom in exact_tree_count but not in exact_tree_count_chunk, which feeds the density grid, so texel sums disagree below the vegetation zoom. Trees are also ungated below their own zoom. |
| T-965 | — | idea | work | Batch waiver matches by substring, not error kind | check.rs waives every error containing the repack phrase; nine distinct wave_lock errors carry it, including a wave listing a ticket with no file. Waive by a typed error kind. |
| T-966 | — | idea | work | world binary feature cannot be taken alone | world/mod.rs and Cargo.toml claim binary can be taken without the parser stack, but lib.rs gates pub mod world on the world feature, so a binary-only consumer fails to compile. |
| T-967 | — | idea | work | ObjectInstancePod pad is documented as zero but unenforced | pod.rs says _pad is always written as 0; a non-zero pad round-trips intact and breaks derived PartialEq. Zero it on construction or exclude it from equality. |
| T-968 | — | idea | work | Oracle 2 label-to-set map is synthetic after relabelling | carry_emptied relabels pending entries, so a re-close of a disavowed label can now corroborate against a set that was never that label's. The shipped test asserts the fixture, not the changed function. |
| T-969 | — | idea | work | clippy map-engine step is not the all-features floor | wave/gate.rs runs clippy with doc, mission and world, omitting png, while claiming to be the all-features floor. |
| T-970 | — | idea | work | Chunk .bin files are neither LFS-tracked nor ignored | packages/map-assets objects/chunks .bin matches no gitattributes or gitignore rule, so about 37 MB of untracked binaries sit in the tree after a real export with nothing stopping a careless add. |
| T-971 | — | idea | work | verify-phase reds on the new chunk .bin files | gates.rs walks a scratch build unfiltered and reports files absent from the committed tree as missing. T-935.2 emits 315 chunk .bin files that are deliberately uncommitted, so verify-phase reports 315 missing. |
| T-972 | — | idea | work | peaks test treats an LFS pointer as a real DEM | dem/peaks.rs returns early when the DEM file is absent but not when it is an LFS pointer, so in a slice worktree it panics on decode instead of skipping. Treat a pointer as absent. |
| T-973 | — | idea | work | land passes a gate base past the wave opening | platform wave land passes the pre-merge HEAD as the gate base. When the command center commits between close and land, that base post-dates the wave opening and the post-merge gate refuses. Derive the base instead. |
| T-974 | — | idea | work | TBDD per-byte ban matches two literal spellings | tbdd.rs bans the strings u16_le and from_le_bytes with a bracket. A shift-or assembler or from_le_bytes with try_into passes. Ban the shape, scoped to the decode body. |
| T-975 | — | idea | work | Density golden fixture pin is skip-shaped | density.rs guards the golden fixture with an exists check, so the pin goes silently vacuous if the file disappears. Make a missing golden a failure. |
| T-976 | — | idea | work | Label host doc names the wrong two requests | world_assets/mod.rs documents LabelHost init as fetching locations.json and road-names.json. On the archive path the count is still two but the files are manifest.json and map_labels.rkyv. One-line doc fix. |
| T-977 | — | idea | work | Town label gates read the JSON source directly | xtask/src/label_gates.rs parses locations.json rather than the manifest, so the gates stay on the JSON after a manifest cutover. Correct while dual emission holds; revisit at the cutover slice. |
| T-978 | — | idea | work | Raw DEM path still materialises one f32 grid | dem_load.rs decodes to u16 then builds an f32 grid because three consumers take a float slice: hillshade, the dem_vectors downsample and the retained grid. Net is 2 buffers against the PNG path's 4; removing the last needs those three files. |
| T-979 | — | idea | work | Shared cache serves stale rmeta to the gate driver | Building xtask in the shared target dir failed on an unresolved import with no compile line for the crate defining it, killing two gates before any step ran. Touching the sources cleared it. |
| T-980 | — | idea | work | T-935.8 verify command filters out its own tests | The ticket's verify line filters on building, which excludes every occluder descriptor test the slice added. A vacuous verify; use the module filter or none. |
| T-982 | — | idea | work | map-blueprint archive subcommand name is wrong | The ticket and docs name cargo xtask map-blueprint archive, which does not exist. The real command is cargo xtask map blueprint-from-voxels archive. Also the ticket says six golden blueprints; the tree has three. |
| T-983 | — | idea | work | Archive sidecars should be fetched with the descriptors | The removed fold ran after the awaited descriptor fetch, so it could add nothing usable. Issuing the archive's sidecar list concurrently with the descriptor round is the actual round-trip saving. |
| T-984 | — | idea | work | New SPA world-asset code is executed by no test | The four frontend files this wave changed are wasm-only, so the native test build compiles them to nothing and no test runs them. About 450 lines of boot logic are compile-checked and never executed. |
| T-985 | — | idea | work | Archive boot drops the hot-set prefetch | Archive boot returns before the blas hot list. FOLDS INTO T-935.13 (wave 244 giant slice). |
| T-986 | — | idea | work | v-suite oracles have drifted on 22 of 25 routes | The frozen route oracles were last refreshed at T-173 while the pages have moved repeatedly since, so gate v-suite verify fails 22 of 25 routes. Only four are clean. Needs a re-freeze or an explicit decision. |
| T-987 | — | idea | work | Frontend wasm clippy carries 76 pre-existing errors | Running clippy on website-frontend for wasm32 with warnings denied reports 76 errors, 121 with all targets. The gate's clippy step is change-scoped so it never sees them. |
| T-988 | — | idea | work | Forest regions file grows 5.7x when smoothed | The smoothed everon rings take forest-regions.json.gz from 43638 to 248877 bytes, 15528 to 42762 vertices. The world load fetches it and no size budget gate covers it. |
| T-989 | — | idea | work | T-149 verify line names a command that does not exist | The ticket says cargo xtask ci verify map-object-golden. That subcommand does not exist; the gate is cargo xtask schema map-object-golden. |
| T-990 | — | idea | work | F6 gate compares raw rings to a smoothed catalogue | The phase gate re-derives forest regions with the raw tracer and compares the array to the committed file. The emit now writes smoothed rings, so they can never match after a regeneration. |
| T-991 | — | idea | work | TbdSatIndexV2 has no schema_version field | Six of the seven binary archives declare a schema version; this one relies on the container header instead. The check is satisfied, but the asymmetry is worth removing. |
| T-992 | — | idea | work | Mip geometry is implemented twice | The level dimension and tile rect arithmetic exists in both the emitter and the reader. Two agreeing implementations is a decent oracle now but a drift hazard; the shared home is the core crate. |
| T-993 | — | idea | work | Satellite boot still hardcodes container v1 | Satellite boot rejects any container but v1. FOLDS INTO T-935.13 (wave 244 giant slice). |
| T-994 | — | idea | work | Forest smoothing emits self-crossing rings | The area-restoring offset moves pinch vertices in opposite directions, so rings cross themselves and their siblings. Measured 0 crossings in, 38 out on the everon corpus. The area check cancels the bowtie exactly and cannot see it. |
| T-995 | — | idea | work | Smoothed forest regions are dropped before render | The world host filters out every region of kind forest, and all 36 everon regions are forest, so no smoothed edge reaches the map. The larger payload is fetched on load and discarded. |
| T-996 | — | idea | work | The v2 satellite verifier checks less than v1 | The v1 verifier cross-checks the index world bounds and terrain against the manifest; the v2 verifier takes no manifest and the v2 index carries neither field. v2 is now the build default. |
| T-997 | — | idea | work | Everon tiling pins never read the committed bundle | Two tests say the expected tiling is read off the bundle, but both compute it from hardcoded constants with no file access. The real bundle does verify, yet no gate step runs that verifier. |
| T-998 | — | idea | work | Container version is read as u16 and u32 | The browser reads a 16 bit version and the tool reads 32 bits, so a v2 container with non-zero flags is version 2 to one and unsupported to the other. |
| T-999 | — | idea | work | Everon road archive is emitted but not committed | The emitter is proven end to end and produces a 435452 byte archive from the committed roads json, but the asset itself lives outside every slice's owns so nothing is committed. The loader slices downstream will have nothing to fetch. |
