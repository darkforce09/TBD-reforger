...
| T-407 | 3223 | running | platform | Fabricated stats still render beside real ones on /me/deployments — the T-392 defect, one page over | Same defect family as T-392 (shipped): mock values sitting on a real user's screen next to real data, indistinguishable from it. T-392 cleared the Event Hub; this is the page it did not own.
...
| T-412 | 3233 | running | platform | GET /api/v1/members cannot return member 21, and list_servers is N+1 over a growing table | Two product-side scaling defects found by T-399's agent during the ratchet sweep. Neither is a test problem; both are live API behaviour.
