...
| T-520 | 3360 | running | platform | W46 hotfix: rustfmt events.rs after T-511 | Wave 46 cold gate FAIL on fmt (changed). T-511 apps/website/api/tests/events.rs fails rustfmt --check (indent/wrapping around the new 23505 probe). Cure: rustfmt that file only; no logic change. Owns: apps/website/api/tests/events.rs. |
