...
| T-726 | 4319 | deferred | eden | Window-Esc pile-up: route every Esc listener through the modal stack | T-691's EditorPreferencesDialog stacks over MissionSettingsDialog and copies the bare window-Esc idiom both dialogs use — one Esc closes BOTH. ui.rs modal_stack::is_topmost_open exists for exactly this and is unused by either dialog. Route both dialogs' Esc through it (topmost closes first, second Esc closes the parent); pin with a stacked-Esc test. Diagnosis: editor_verify/wave106.md MINOR-2. WIDENED wave 108 (MAJOR-2): the ruler's Esc arm is another unguarded window-keydown that never consults the T-333 one-Esc-one-dialog stack — deterministic repro: place a ruler, RMB (context menu opens), Esc to close the menu → the menu closes AND the placed measurement clears; the two-step Esc escalation is defeated by the first overlay. The fix is one design across ALL window-Esc listeners (both dialogs, context menu, asset picker, ruler): consult modal_stack::is_topmost_open / register as stack entries, topmost consumes. |
