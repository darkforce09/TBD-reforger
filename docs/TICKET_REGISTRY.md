...
| T-151 | 1500 | queued | Platform | WebGPU (wgpu/wasm) render engine spike - replace Deck.gl | Spike a pure wgpu (WebGPU + WebGL2 fallback) renderer compiled to wasm that reads the yrs data core directly and replaces Deck.gl - driven by the IconLayer binary-attribute limit that blocks zero-copy world-object render through Deck. |
