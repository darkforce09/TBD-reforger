...
| T-449 | 3288 | deferred | platform | Wire ./scripts/ticket check to .ai/tickets/schema.json | Residual from T-273 / wave 15 adversarial NIT (also T-237 territory). Regenerated schema validates 423 tickets via python jsonschema, but xtask/src/check.rs never loads .ai/tickets/schema.json — wave gate ticket registry PASS does not examine the schema file.
