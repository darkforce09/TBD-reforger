...
| T-524 | 3364 | running | platform | T-505 compile.rs KNOWN keys / hydrate-title comment lockstep | Wave 46 verifier NIT. After T-505, store.rs loads payload title and lists title as known, but crates/map-engine-core/src/mission/compile.rs still omits title from KNOWN_EDITOR_PAYLOAD_TOP_LEVEL_KEYS and retains stale T-375 comment that hydrate does not load title. Cure: add title to known keys + fix comment. |
...
| T-549 | 3389 | running | platform | Live /compiled still uses empty CargoPhysCatalog (T-500 trust-saved residual) | FOUND by W58 adversarial verifier (CLEAN MINOR) after T-500.
