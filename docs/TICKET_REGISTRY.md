...
| T-524 | 3364 | deferred | platform | T-505 compile.rs KNOWN keys / hydrate-title comment lockstep | Wave 46 verifier NIT. After T-505, store.rs loads payload title and lists title as known, but crates/map-engine-core/src/mission/compile.rs still omits title from KNOWN_EDITOR_PAYLOAD_TOP_LEVEL_KEYS and retains stale T-375 comment that hydrate does not load title. Cure: add title to known keys + fix comment. |
| T-525 | 3365 | deferred | platform | content_golden.sql obsolete two-seat / index-deferred comments after T-511 | Wave 46 verifier NIT. apps/website/api/seeds/content_golden.sql ~655–656 still says orbat assigned partial unique deferred / events.rs still seeds two-seat — obsolete after 0017 + T-511 seed retirement. Cure: update seed comments to match live index. |
...
| T-537 | 3377 | running | platform | xtask empty/lossy write residuals outside T-383 owns (enf/world/map) | FOUND as T-383 residual (W51 CLEAN). Owns fixed cmds/sync/schema_gates; still open:
