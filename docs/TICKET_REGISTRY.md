...
| T-261 | 2790 | ready | platform | Leaderboard materialized view has no scheduled refresh | refresh_leaderboard has exactly one caller, inside results ingest. If ingest never fires the ladder is frozen at the WITH NO DATA state from migration 0002 forever. No cron, no interval, no admin refresh button. |
