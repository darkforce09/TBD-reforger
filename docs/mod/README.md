...
**Verification scripts:** `scripts/mod/mcp-call-selftest.sh` (offline MCP), `scripts/mod/mcp-smoke.sh` (live Workbench), `cargo xtask mod spawn-determinism` (`make mod-spawn-determinism`), `cargo xtask mod spawn-verify`, `scripts/mod/remote-log-grep.sh` (staging), `cargo xtask debug direct-join` (LAN join)
