//! `gate doctor` — a fail-fast preflight for the editor CDP smokes.
//!
...
//! (The actual root cause: `chrome-headless-shell`'s stubbed Skia font manager FATAL-crashes on
...
/* ──────────────── The gate owns its fontconfig cache ──────────────── */
...
/// What `check_fonts` learned. The three states are not decoration: "chromium told us it
...
enum FontProbe {
    Ok,
    /// The marker fired: chromium resolved zero fonts — the wedge, pre-crash.
    NoFonts,
    /// The probe itself failed (no log, no spawn, chromium exited early) — verdict unknown.
    Inconclusive,
}

/// What the liveness probe found. `BrowserDied` is the crash case: a CDP call never answers because
