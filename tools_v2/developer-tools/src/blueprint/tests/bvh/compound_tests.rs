...
/// never traced it) replayed against the 4000-pair door-inclusive oracle.
/// Measured 2026-09-04: 3983/4000 agree (re-blessed to 3998/4000 the same day under the
/// Projectile layer policy, which drops the shell's `Building` physics mesh in
...
fn farmhouse_compound_door_parity_is_pinned() {
    ...
    // 132 → 120: the projectile layer policy left the twelve `LightSwitch_02`
    ...
    // The oracle (doors and glass excluded) is untouched by the instances.
    assert_eq!(
