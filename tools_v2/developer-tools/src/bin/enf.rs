...
use developer_tools::enf::{apidoc, capability, carve, citations, index, source};
use developer_tools::enfusion_pak::PakVfs;
    ...
    match cli.cmd {
        ...
        Cmd::Citations { docs, index_dir } => {
            ...
            if !rep.unresolved.is_empty() {
                ...
                eprintln!(
                    "(rebuild: cargo run -q -p developer-tools --bin enf -- index crf / cargo run -q -p developer-tools --bin enf -- carve / cargo run -q -p developer-tools --bin enf -- apidoc)."
                );
                return Ok(ExitCode::from(1));
            }
