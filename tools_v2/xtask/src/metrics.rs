...
//! ([`ticket_engine::validate_rfc3339_utc`]).
//!
...
fn parse_utc(field: &str, value: &str) -> Result<OffsetDateTime> {
    ticket_engine::validate_rfc3339_utc(field, value).map_err(|e| anyhow::anyhow!("{e}"))?;
...
//
// Pinned by fixtures in `tools_v2/xtask/tests/fixtures/` (recorded output, not guessed):
...
pub fn stamp_land(root: &Path, id: &str, land_sha: &str) -> Result<PathBuf> {
    stamp_land_at(root, id, land_sha, &ticket_engine::now_utc_rfc3339())
}
