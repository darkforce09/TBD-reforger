...
/// by the typed ops (`ticket_engine::ops` + `Corpus::write_back` in `cmds.rs`); this fn REFUSES
...
// projection anymore — writers go through `ticket_engine::ops` on the typed corpus, and the
