...
use ticket_engine::{
    Corpus, ScopeVocab, Ticket, TicketFile, parse_ticket_toml, render_ticket_toml,
};
    ...
    for path in &paths {
        ...
        if kind == "work" {
            ...
            table.insert(
                "class".into(),
                toml::Value::String(
                    ticket_engine::classify_work(&format!("{title} {summary}")).to_string(),
                ),
            );
...
fn print_scope_histogram(corpus: &Corpus) {
    ...
    let mut buckets: BTreeMap<String, Vec<&ticket_engine::WorkTicket>> = BTreeMap::new();
