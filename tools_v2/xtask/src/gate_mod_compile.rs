...
pub fn run_with_root(root: &Path, opts: &Opts) -> u8 {
    if require_host().is_err() {
        return env_fail(
            ...
            Some(
                "See tools_v2/xtask/src/hostrun.rs: the container has no C toolchain and an older glibc, so the game binary cannot run in here at all.",
            ),
        );
    }
...
fn walk_files(base: &Path) -> io::Result<Vec<PathBuf>> {
...
fn count_game_scripts(addon: &Path) -> io::Result<u64> {
...
fn workbench_tooling_guard(mod_src: &Path) -> Vec<String> {
