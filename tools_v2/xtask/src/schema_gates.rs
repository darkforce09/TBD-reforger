...
///   2. against `developer_tools::world::INSTANCE_KINDS`, order included — the two copies exist because
...
fn instance_kinds_lockstep_failures(enums: &Value) -> Vec<String> {
        ...
        (Some(all), Some(regions)) if !all.is_empty() && !regions.is_empty() => {
            ...
            if actual_ref != expected {
                ...
                let spurious: Vec<&str> = actual_ref
                    ...
                    .collect();
                out.push(format!(
                    "INSTANCE_KINDS (tools_v2/xtask/src/schema_gates.rs) drifted from \
    ...
    if INSTANCE_KINDS[..] != developer_tools::world::INSTANCE_KINDS[..] {
        out.push(format!(
            "INSTANCE_KINDS (tools_v2/xtask/src/schema_gates.rs) {:?} != \
             developer_tools::world::INSTANCE_KINDS {:?} — the two census kind lists must stay \
             identical INCLUDING ORDER (it is the emitted byKind key order)",
            INSTANCE_KINDS,
            developer_tools::world::INSTANCE_KINDS
        ));
    }
    out
}
...
mod t212_side_fallback_tests {
    ...
    fn invalid_side_is_neutral_but_absent_and_valid_sides_keep_their_roles() {
        let root = repo_root().expect("repo root");
        let lane = root.join("apps/mod/tbd-framework/Scripts/Game/TBD/Objectives");
...
"#,
        ...
        program
            .push_str("static void CheckTypedSide(Objective& objective, Row row, string subject)");
...
"#,
        ...
        let dir = std::env::temp_dir().join(format!("t212-side-fallback-{}", std::process::id()));
        ...
        let compile = Command::new("c++")
            ...
            .arg(&input)
            .output()
            .expect("C++ compiler for Enforce source simulation");
        assert!(
