...
//! [`ticket_engine::Corpus::write_back`] only — estimate file ⇔ marker, both
...
use ticket_engine::{Corpus, StatusName, Ticket, validate_rfc3339_utc};
use walkdir::WalkDir;
...
fn loc_of(sha_loc: &BTreeMap<String, u64>, sha: &str) -> u64 {
...
pub fn cmd_estimate_tokens(root: &Path) -> Result<()> {
    ...
    let report = run_estimates(root, &subjects, &sha_loc, &ticket_engine::now_utc_rfc3339())?;
    if report.records.is_empty() {
...
mod tests {
    use super::*;
    use serde_json::json;
    use ticket_engine::{Domain, ProgramTicket, ScopeV2, Status, WorkTicket};
