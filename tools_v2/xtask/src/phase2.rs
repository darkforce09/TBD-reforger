...
use std::path::Path;
use ticket_engine::{Domain, ScopeV2, Ticket, TicketFile, parse_ticket_toml, render_ticket_toml};
...
/// mutators write through `ticket_engine::ops` + `Corpus::write_back` now, so no mutator path
...
/// `ticket_engine::ops` + `Corpus::write_back` (surgical per-file temp+rename writes); nothing
...
mod tests {
    ...
    ///    verb writes through `ticket_engine::ops` + `Corpus::write_back`. Needle assembled at
