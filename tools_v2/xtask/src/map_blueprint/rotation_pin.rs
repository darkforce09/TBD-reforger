...
//! (`tools_v2/xtask/tests/fixtures/rotation_pin_*.json`, captured from `recon` + the prefab text).
...
mod tests {
    ...
    fn garbage_container_lid_pins_y_x_z_with_negated_pitch_and_roll() {
        ...
        let fx = load_fixture(
            &root.join("tools_v2/xtask/tests/fixtures/rotation_pin_GarbageContainer_01.json"),
        )
        .unwrap();
        let scores = score_all(&fx);
        let winner = &scores[0];
