...
fn check_open_work_owns(root: &Path) -> Vec<String> {
    let corpus = match ticket_engine::Corpus::load(root) {
        Ok(c) => c,
        Err(e) => return vec![e],
    };
    let mut errors = Vec::new();
    for ticket in corpus.tickets.values() {
        if let ticket_engine::Ticket::Work(w) = ticket {
    ...
    for ticket in corpus.tickets.values() {
            ...
            && w.class.is_none()
        {
            errors.push(format!(
                ...
                w.id,
                ticket_engine::CLASS_VALUES.join("|")
            ));
        }
    }
    errors
...
fn check_live_work_surface(root: &Path) -> Vec<String> {
    ...
    let vocab = match ticket_engine::ScopeVocab::load(root) {
        Ok(v) => v,
        Err(e) => return vec![e],
    };
    let mut errors = Vec::new();
...
fn body_findings(corpus: &ticket_engine::Corpus) -> (Vec<String>, Vec<String>) {
    use ticket_engine::{BODY_LINE_WORD_CAP, CITATION_WORD_CAP, SUMMARY_WORD_CAP, Ticket};
    let mut errors = Vec::new();
...
fn check_estimated_stamp_coherence(root: &Path) -> Vec<String> {
    ...
    for (id, ticket) in &corpus.tickets {
        let (created, completed, shipped, estimated, note) = match ticket {
            ticket_engine::Ticket::Program(p) => {
                let shipped = match &p.status {
                    ticket_engine::Status::Shipped { shipped_at, .. } => shipped_at.as_deref(),
                    _ => None,
                };
                (
...
///   `ticket_engine::is_sha_shaped` authority) — OR absent with `"shipped_at"` in
...
fn check_ship_gate(root: &Path) -> Vec<String> {
    ...
    let mut errors = Vec::new();
    for (id, ticket) in &corpus.tickets {
        if ticket.status().name() != ticket_engine::StatusName::Shipped {
            continue;
        }
        ...
        match shipped {
            Some(v) if ticket_engine::is_sha_shaped(v) => {}
...
fn check_plan_ready_gate(root: &Path) -> Vec<String> {
    ...
    let mut errors = Vec::new();
    for (id, ticket) in &corpus.tickets {
        let ticket_engine::Ticket::Work(w) = ticket else {
            continue;
        };
        if !matches!(
            w.status.name(),
            ticket_engine::StatusName::Ready
                | ticket_engine::StatusName::Running
                | ticket_engine::StatusName::Review
        ) {
            continue;
        }
        let status = w.status.name().as_str();
...
/// [`ticket_engine::TITLE_DEBT_PIN`] — see [`check_debt_pins`] — and are enforced on
...
/// ([`ticket_engine::empty_ready_tier_fields`] — context, requirement, current_state,
    ...
    for ticket in corpus.tickets.values() {
        ...
        if !matches!(
            ...
            ticket_engine::StatusName::Ready
                | ticket_engine::StatusName::Running
                | ticket_engine::StatusName::Review
                | ticket_engine::StatusName::Shipped
        ) || !w.migration_legacy.is_empty()
        {
            continue;
        }
        let missing = ticket_engine::empty_ready_tier_fields(w);
        if !missing.is_empty() {
            errors.push(format!(
...
/// (`ticket_engine::title_is_debt` / `main_goal_is_debt`) — split pure so the
/// fixture tests and the counter printer consume the same arithmetic.
fn debt_counts(corpus: &ticket_engine::Corpus) -> (usize, usize) {
    let mut title = 0usize;
    let mut main_goal = 0usize;
    for (id, t) in &corpus.tickets {
        match t {
            ticket_engine::Ticket::Program(p) => {
                if ticket_engine::title_is_debt(id, &p.title) {
                    title += 1;
                }
            }
            ticket_engine::Ticket::Work(w) => {
                if ticket_engine::title_is_debt(id, &w.title) {
                    title += 1;
                }
                if ticket_engine::main_goal_is_debt(w) {
                    main_goal += 1;
                }
            }
        }
...
fn debt_counter_lines(root: &Path) -> Option<Vec<String>> {
    let corpus = ticket_engine::Corpus::load(root).ok()?;
    let (title, main_goal) = debt_counts(&corpus);
...
const FOSSIL_ALLOWLIST: &[(&str, &str)] = &[
    ...
    (
        "tools_v2/xtask/src/wave/legacy_plan.rs",
    ...
    for path in String::from_utf8_lossy(&out.stdout).lines() {
        ...
        errors.push(format!(
            "dead wave-plan reference in {path} — the TSVs and their env knobs died at T-912.2; \
             ...
             tools_v2/xtask/src/check.rs, with a reason)"
        ));
    }
    errors
}
...
fn strict_honesty_counters(root: &Path) -> Option<Vec<String>> {
    ...
    for (id, ticket) in &corpus.tickets {
        ...
        let (estimated, note) = match ticket {
            ticket_engine::Ticket::Program(p) => (&p.estimated, p.estimate_note.as_deref()),
            ticket_engine::Ticket::Work(w) => (&w.estimated, w.estimate_note.as_deref()),
        };
...
"#;
    ...
    fn body_caps_red_green_and_warning_channel() {
        ...
        let wall41 = (1..=41)
            ...
            .collect::<Vec<_>>()
            .join(" ");
        let load = |tmp: &Path| ticket_engine::Corpus::load(tmp).expect("scratch corpus loads");
    ...
    fn quarantine_mint_past_cutover_is_red() {
        ...
        .unwrap();
        let (errs, _) = body_findings(&ticket_engine::Corpus::load(&tmp).unwrap());
        assert_eq!(errs.len(), 1, "{errs:?}");
        assert!(
            errs[0].contains("T-001")
