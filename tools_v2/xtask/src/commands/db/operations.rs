...
//! can work (see `crate::commands::db::operations::selftest`).
//!
...
pub fn run(cmd: DbCmd) -> Result<u8> {
    match cmd {
        ...
        DbCmd::Backup { db, out, keep } => {
            ...
            crate::commands::deploy::database_backup::run(&argv)
        }
        ...
        DbCmd::BackupDrill { db, out, fresh } => {
            ...
            crate::commands::deploy::database_restore_drill::run(&argv)
        }
        DbCmd::BackupVerify { dump } => match dump.as_deref() {
            ...
            Some(d) => crate::commands::deploy::database_backup::run(&[
...
fn restore(dump: Option<&str>, db: Option<&str>, create: bool) -> Result<u8> {
    ...
    struct Argv {
        #[command(flatten)]
        inner: crate::commands::deploy::database_restore::RestoreArgs,
    }
    let parsed = Argv::parse_from(argv);
    crate::commands::deploy::database_restore::run(parsed.inner)
}

#[cfg(test)]
#[path = "tests/operations/tests.rs"]
mod tests;
