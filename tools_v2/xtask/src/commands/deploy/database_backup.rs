...
use crate::commands::deploy::database_operations::{
    count_db_rows, ct_i_to_files, database_exists, db_container, db_user, die, info,
...
fn run_verify_only(file: &Path, min_rows: u64, db: &str) -> Result<u8> {
    ...
    match verify_dump(file, min_rows, Some(db)) {
        ...
        Err(crate::commands::deploy::database_operations::VerifyFail) => {
...
#[cfg(test)]
#[path = "tests/database_backup/tests.rs"]
mod tests;
