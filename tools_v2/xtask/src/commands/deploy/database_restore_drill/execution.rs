...
pub fn run(args: &[String]) -> Result<u8> {
    ...
    if fresh {
        ...
        let rc = crate::commands::deploy::database_backup::run(&[
            "--db".into(),
            source_db.clone(),
            "--out".into(),
            out.clone(),
...
pub(super) fn print_usage(to_stderr: bool) {
    let text = "\
...
pub(super) fn default_restore_jobs() -> u32 {
...
pub(super) fn default_restore_min_rows() -> u64 {
...
pub(super) fn dump_age_hours(dump: &Path) -> u64 {
...
pub(super) fn mig_ver(path: &Path) -> String {
