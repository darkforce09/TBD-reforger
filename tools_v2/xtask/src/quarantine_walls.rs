...
use std::fs;
use std::path::Path;
use ticket_engine::{Corpus, SUMMARY_WORD_CAP, Ticket};
...
mod tests {
    ...
    use ticket_engine::{Domain, ScopeV2, Status, WorkTicket, render_ticket_toml};
    ...
    fn parked_ticket_renders_canonically() {
        ...
        let reparsed = ticket_engine::parse_ticket_toml(&disk).expect("landed file parses");
        assert_eq!(
