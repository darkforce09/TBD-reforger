...
use ticket_engine::{Corpus, Ticket, ops};
...
pub fn cmd_ship_opt(root: &Path, registry: &mut Value, id: &str, refresh: bool) -> Result<()> {
    ...
    let outcome = ops::ship(&mut corpus, id, &ticket_engine::now_utc_rfc3339())
...
pub fn cmd_remove(root: &Path, registry: &mut Value, id: &str, force: bool) -> Result<()> {
    ...
    let outcome = ops::remove(&mut corpus, id, force, &ticket_engine::now_utc_rfc3339())
        .map_err(anyhow::Error::msg)?;
    corpus
        .write_back(&outcome.changed)
        .map_err(anyhow::Error::msg)?;
...
pub fn cmd_reorder(root: &Path, registry: &mut Value, id: &str, after: &str) -> Result<()> {
    ...
    let outcome = match ops::reorder(&mut corpus, id, after, &ticket_engine::now_utc_rfc3339()) {
        Ok(o) => o,
        Err(msg) => refuse_verbatim(&msg),
    };
...
pub fn cmd_advance_slice(root: &Path, registry: &mut Value, id: &str) -> Result<()> {
    ...
    let outcome = match ops::advance_slice(&mut corpus, id, &ticket_engine::now_utc_rfc3339()) {
        Ok(o) => o,
        Err(msg) => refuse_verbatim(&msg),
    };
...
pub fn cmd_set_status(root: &Path, registry: &mut Value, id: &str, status: &str) -> Result<()> {
    ...
    let outcome = ops::set_status(&mut corpus, id, status, &ticket_engine::now_utc_rfc3339())
        .map_err(anyhow::Error::msg)?;
    corpus
        .write_back(&outcome.changed)
        .map_err(anyhow::Error::msg)?;
...
mod tests {
    ...
    fn scratch_registry(tag: &str) -> PathBuf {
        use ticket_engine::{Domain, ProgramTicket, ScopeV2, Status, Ticket, WorkTicket};
        ...
        let work =
            |id: &str, status: Status, spec: Option<&str>, parent: Option<&str>, owns: &[&str]| {
                ...
                Ticket::Work(WorkTicket {
                    ...
                    plan: ready_class.then(|| ticket_engine::ops::default_plan_path(id)),
                    depends_on: vec![],
                    unblocks: vec![],
    ...
    fn parse_scratch_ticket(root: &Path, id: &str) -> ticket_engine::Ticket {
        ticket_engine::parse_ticket_toml(
    ...
    fn child_ship_end_to_end_typed_path() {
        ...
        match parse_scratch_ticket(&root, "T-001.2") {
            Ticket::Work(w) => {
                assert_eq!(
                    w.status,
                    ticket_engine::Status::Deferred { order: None },
