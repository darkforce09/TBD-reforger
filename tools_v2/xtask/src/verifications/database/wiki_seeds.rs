...
//! inspect. The successor is [`crate::commands::db::operations::SEEDS`] — the const `cargo xtask db seed` iterates,
...
fn first_failure(repo_root: &Path, seeds: &[&str]) -> Result<Verdict> {
    ...
    // `crate::commands::db::operations::seed()` or is not in the slice. The whole comment-smuggle class is unreachable here
