...
//! folders — and files each in one area for the totals, telling frozen records from live
//! documents.
//!
...
//! order of [`DocumentArea::ALL`]; the program records at the documentation root are never
//! judged; the ticket records and the archive are the frozen areas.
...
use crate::core::repository_layout::documentation::{
    ARCHIVE_DIR, CURSOR_RULE_DIRS, DOCUMENTATION_ROOT, PROGRAM_RECORDS_PREFIX,
...
pub(super) enum DocumentArea {
    ...
    ProjectInstructions,
    /// A README.md outside the areas above, the repository root's included.
    Readmes,
}
