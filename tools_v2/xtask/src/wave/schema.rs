...
//! anyway: these gates read `tools_v2/xtask/src/schema_gates.rs`, `packages/tbd-schema/rules/`,
...
pub fn gate_schema(ctx: &Ctx) -> i32 {
    ...
    if mk_gates.is_empty() {
        wprintln!(
            "schema: read 0 sub-gates out of the schema-validate task (tools_v2/xtask/src/mk_ci_tasks.rs)."
        );
        wprintln!(
