...
- `src/verifications/` groups checks by architecture, licensing, prohibited languages, mod scripts, deployment, registries, database seeds, CI, schemas, map assets, documentation, and API readiness. Map-asset checks delegate engine work to `developer-tools`. The documentation group holds the README-coverage, Markdown-placement and link-check gates. The API-readiness group passes only when every requirement in the API readiness register has current evidence, each receipt fingerprinted against the source and the effective configuration.
